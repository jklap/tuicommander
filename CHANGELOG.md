...
- **Consent prompt for wrapping your own `claude`/`codex`/`goose` shell function (zsh only).** If your `.zshrc` already defines its own function for one of these (e.g. to pick a model), TUIC's launch-flag injection (`--settings`/`-c notify`/`--name`) used to be silently skipped for that shell — no error, no signal anything was missing, just slower screen-scraping status detection instead of the hook-driven kind. The first time this is detected, a dialog now asks whether to wrap your function (call it, then append TUIC's flag) or leave it alone — explaining that wrapping only works if your function forwards its arguments (`"$@"`), and that if your function already sets its own version of the same flag, the CLI silently keeps only the last one, so TUIC's would replace yours with no warning. Your choice (or "ask me later") is remembered per agent in Settings → Agents → expand an agent → "If your shell already defines its own ⟨agent⟩ function" (a three-state control: Ask when detected / Wrap my function / Leave my function alone). Applies to new terminals only.
- **Session State Explain — a troubleshooting dump for why a session's status badge is what it is.** A session's tab occasionally reads "idle" while an agent is still working, or "working" after it's already finished — reachable from the Activity Dashboard row (a small "?" button) or a terminal tab's right-click context menu ("Explain State…"). Reports the ranked evidence held for busy/idle/awaiting, whether the internal arbiter would flip the shell state right now, which specific rule of the idle/working/awaiting logic produced the current badge, and an always-on decision trail of the last 64 evidence updates — including attempts that were rejected because stronger contradicting evidence was already held, and exactly what that evidence was. Also available over HTTP (`GET /sessions/{id}/explain-state`) and MCP (`debug` tool, `explain_state` action) for scripting or remote diagnosis. A "Copy as JSON" button on the modal makes it easy to paste into a bug report.
- **Session Diff Review — step through everything a Claude Code session changed.** A new tab (Command Palette: "Session diff review") reconstructs a session's edit history from its transcript and, where available, its pre-edit file backups — no new agent instrumentation required. Two views: grouped-by-file (default — a cumulative diff per file, with its individual edits collapsible underneath) or flat chronological order. Defaults to the focused terminal's live agent session, with a picker to review a different past session for the repo. Per step or file: open the file at that change, comment on selected lines (sent to the agent, same format as the regular Diff tab), copy the diff, and revert — a single step (keeping every later edit) or a whole file back to its session-start content. A file whose starting content can't be determined, or that's changed outside the session since, is flagged rather than guessed at. Subagent edits are included by default, with a toggle to exclude them.
- **Setup/Archive/Run scripts, and Smart Prompt shell/headless children, now get a `TUIC_*` environment** describing the worktree they're running in — `TUIC_MAIN_REPO_PATH`, `TUIC_BRANCH`, `TUIC_BASE_REF`, `TUIC_BASE_BRANCH`, `TUIC_WORKTREE_PATH`/`_NAME`/`_DIR`, `TUIC_IS_WORKTREE`, `TUIC_REPO_NAME`, plus `TUIC_SCRIPT_KIND`/`TUIC_APP_VERSION`/`TUIC_CONFIG_DIR` — so a setup script no longer has to guess the main checkout from `pwd`. Setup/Archive scripts also now get the same `PATH` enrichment every git subprocess already had (fixes `npm install`-style commands failing under a GUI-launched app's minimal `PATH`). Documented in the AI Agents guide's new "Worktree Context" section.
- **Smart Prompts context variables gained a Worktree section** (`{worktree_path}`, `{main_repo_path}`, `{worktree_name}`, `{is_worktree}`) and `{session_id}`; `{repo_name}`/`{repo_path}` now describe the *tree the variables were resolved against* (the focused terminal's worktree, not just "the active repo" — see Fixed below).
- **`POST /worktrees/run-script`** — the HTTP/MCP counterpart to the `run_setup_script` Tauri command now exists (previously mapped by the frontend but 404'd over HTTP/remote/browser mode).
- **`repo action=worktree_setup_status` (MCP) / `GET /worktrees/setup-status` (HTTP)** — poll a worktree's Setup Script outcome (`{state: "running" | "not_configured" | "completed" | "unknown"}`, plus `exit_code`/`error` once completed). Closes the gap left by worktree creation no longer returning `setup_script`/`setup_script_error` synchronously: an MCP client has no event stream to receive `worktree-setup-script-completed` on, so this lets it poll instead.
- **Additional Readable Directories** (Settings → Services & MCP → File Access) — a browser/remote/PWA client can now read files from user-configured absolute directories, in addition to registered repository roots. Ships with `~/.claude/plans` enabled by default, so clicking a Claude Code plan-file link an agent printed now works in browser/remote mode with no setup. Read-only: this never widens what a remote client may write, copy, or move — those routes stay confined to registered repository roots.
- **StreamDock M18 macropad support** — Settings > StreamDock lets you attach a 15-key LCD macropad (5×3 keys + 3 plain buttons) that mirrors live session state: each key shows a session's status (working/idle/needs input/error/completed, its short alias, and current task) and pressing it focuses that tab. Tap answers a pending choice prompt or focuses the tab; double-tap answers the second option; hold sends an interrupt. Sessions keep a stable key across the panel's lifetime — a closed session's key goes blank rather than reindexing its neighbors — and a session awaiting input is never displaced from a key by another one also awaiting input. Settings gains device selection (with a rescan button for when more than one is connected), screen/LED brightness sliders, and per-session pinning so a chosen session's key is never evicted. On firmware that negotiated RGB support, the 24-LED ring also shows one ambient summary color for the whole panel — green/peach/red for nothing pending/needs input/error — updated only when it actually changes. Unplug/replug recovers automatically. Desktop only.

### Changed

- **Worktree creation's file sync now always finishes before its Setup Script runs** (previously unsequenced by design — a script depending on a `copy_ignored_files`/`copy_untracked_files`/`copy_paths`-synced file could run before the sync had a chance to start). Both steps still run in the background after worktree creation itself returns, so this doesn't reintroduce blocking — only the *order* changed. As part of this, the desktop app's worktree-creation response and the MCP `repo worktree create` tool's response **no longer include `setup_script`/`setup_script_error`**: a configured setup script's outcome is now reported via a new `worktree-setup-script-completed` event instead of a synchronous field, or can be polled via the new `repo action=worktree_setup_status`/`GET /worktrees/setup-status` (see Added above). `mcp_http/session.rs`'s worktree+session creation endpoint also now runs the file sync for the first time — previously a separate, unrelated gap where it never synced at all.
- A Setup or Archive script that hangs is now killed after a timeout (default 600s for Setup, 120s for Archive, configurable via the global repo defaults — no Settings UI for these two fields yet) instead of blocking forever. On Unix the whole process group is killed, not just the shell, so a hung `npm install`'s own children are cleaned up too. A timed-out Archive script now aborts the archive/remove (surfaces as an ordinary script-failure error); a timed-out Setup script returns an error rather than the same `exit_code: -1` a signal-killed script reports.

### Fixed

- **Smart Prompts could resolve variables against the wrong tree.** `{branch}`/`{diff}`/etc. used to resolve against the last-focused *repo*, while the command they were substituted into ran in the active *terminal's* actual directory — so with a worktree tab focused, Smart Commit could generate a commit message from the main checkout's diff and then commit it in the worktree. Variables now resolve against the tree that owns the focused terminal's cwd, falling back to the previous behavior when that cwd belongs to no registered repo.
- Two documentation files (`prompt-library.md`, `FEATURES.md`) incorrectly showed Smart Prompts variables using `{{double-brace}}` syntax — no code path has ever supported this (`{{diff}}` is parsed as the literal variable name `{diff`, which never resolves). Docs corrected to the single-brace syntax that has always been required; if you have a saved prompt using the old syntax, it still needs a manual edit — this was a docs-only bug.
- **The Run Script could start before the Setup Script finished, again** — the background-chain ordering fix above (sync always finishes before the setup script runs) fixed the sync-vs-script race, but removed the frontend's old inline `await runSetupScript(...)` with no replacement, so the Run Script (typed into the new terminal) could still start concurrently with — or before — the Setup Script, e.g. `npm run dev` racing `npm install`. The frontend now waits for the `worktree-setup-script-completed` event before creating the terminal whenever a setup script is configured, with a generous timeout so a lost event can't hang worktree creation forever.
- **A subdirectory cwd (e.g. a Finder Service tab opened inside a worktree subfolder) got no `TUIC_*` context vars at all** — the repo-root lookup only ever checked its exact given path, never walked up to find the owning worktree/repo the way `git rev-parse --show-toplevel` does. Fixed; `TUIC_*` now describes the owning repo root even when the script/terminal's cwd is a subdirectory of it.
- **`{base_branch}` in Smart Prompts could disagree with a worktree script's `$TUIC_BASE_BRANCH` for the same repo** — the Smart Prompts variable ignored a repo's configured "Branch From" override and always auto-detected instead. Both now resolve through the same function.
- **The Git panel's "Generate commit message" could generate from the wrong repo's diff.** It resolved Smart Prompts variables (and, since this is a headless prompt, its own execution directory) against the active *terminal's* repo/worktree rather than the specific repo the Git panel itself was showing — so with a different repo's terminal focused, it could generate a message from that repo's diff and commit it (always against the panel's own repo) into the wrong place. `executeSmartPrompt` now accepts an explicit target path a caller can pin to its own repo/worktree, independent of terminal focus; the Git panel uses it.
- **Every Smart Prompt button in a GitHub/PR-related panel had the same wrong-repo risk as the commit-message bug above** — `SmartButtonStrip` (used by the Issues/PR popovers, the Branches tab's "Create PR", and the Changes tab's own commit-button strip) took a `repoPath` prop but never forwarded it, so it also fell back to resolving against the active terminal's repo. Fixed in the one shared component, covering all five call sites at once.
- **A browser/remote client opening a file outside its registered repositories used to fail with a raw RPC string** (`RPC read_external_file failed: 403 {"error":...}`) dumped into both the Markdown/code-editor tab's error banner and an `error`-level log line — for a case that's neither a real error nor something the user can't fix. Now shows a friendly, actionable message and logs quietly, in both `MarkdownTab` and `CodeEditorTab`.
- **Notification sounds are now customizable per event** — Settings > Notifications gains a source picker for each event (Question, Error, Completion, Warning, Info, Attention): keep its own default tone, borrow another event's tone as a preset, or supply a custom audio file (`.wav`/`.mp3`/`.ogg`/`.flac`) via a native file picker. A custom file that can't be opened or decoded falls back to that event's default tone rather than staying silent. Browser/PWA mode supports the borrowed-preset option but not custom files (no filesystem access there), falling back the same way.
- **iTerm2 OSC 1337 proprietary escape codes are now recognized**: `CursorShape`, `ClearScrollback`, `Copy`/`CopyToClipboard`+`EndCopy` (clipboard writes — the same permission gate as OSC 52), `StealFocus` (brings the window to the front), `RequestAttention` (bounces the dock icon), and `OpenURL`. `StealFocus`/`RequestAttention` are gated by a new Settings > Terminal > Behavior toggle, "Allow terminal focus/attention requests" (default on) — any process writing to the PTY could otherwise yank focus or spam the dock icon. `OpenURL` always shows a confirmation dialog before opening (reusing the same confirm flow MCP tools use, so it's answerable from any connected client, not just the desktop app) and only ever opens http/https/mailto URLs.
- **The `tuic`-as-`tmux` compatibility shim now supports the pane/window graph tools like Claude Code's teammate-pane backend actually drive** — `-V`, `new-window`, `split-window`, `respawn-pane`, `list-windows`, `list-panes`, `display-message`, `select-pane -T`, and `kill-pane` were previously unsupported or stubbed; a real pane/window/session graph now lives app-side (partitioned by tmux's own `-L`/`-S` global options, exactly as two concurrent tmux-driving processes expect), reconciled against live sessions so closing a tab by hand doesn't leave a dangling pane reference behind. Every invocation — including ones this layer doesn't recognize — is now logged (`<config dir>/logs/tmux-shim.log` and, when the app is running, `POST /logs`), toggle with `TUIC_TMUX_LOG`. `select-pane -T` also now actually renames the tab: `PUT /sessions/:id/name` previously mutated the session's display name with no event emitted at all, so a rename (from any source, not just this shim) was invisible in the UI until the next full app restart.
- **The OSC 9;4 progress bar now distinguishes error, warning, and indeterminate states, not just a percentage** — previously any non-zero state rendered identically to a normal progress fill. Error and the new iTerm2-style warning state (4) now render in their respective status colors, each optionally at a reported percentage (a full-width bar when the emitter sends no percentage, since not every error/warning report knows one). Indeterminate now ignores whatever percentage it's sent — there often isn't a meaningful one — and instead plays a continuous sweep animation across the full width of the bar, on both the desktop tab bar and the mobile session card/detail header.
- **Smart Selection rules can now be exported and imported, and both Smart Prompts and Smart Selection exports use a native Save dialog** — Settings → Selection gains the same Export/Import toolbar Smart Prompts already had (a scope dropdown — All rules / Modified only / Custom only — plus Export…/Import…), letting you move your tuned regex rule set to another machine or share it with a teammate. A rule is classified as "modified" against the built-in default matching its id (order-sensitive, since action order sets the right-click menu order and which action is the default); importing shows every rule as NEW or CONFLICT in a review dialog before anything is applied, and a rule carrying a Run Command, Run Command in New Terminal, or Send Text action always imports disabled, pending review — the same treatment a shell/API Smart Prompt already gets. Both features' Export now opens the OS Save dialog (previously a silent browser download to a fixed filename) so you choose the name and location; browser/PWA mode falls back to the previous download behavior, since the native dialog isn't available there. Smart Prompts' export scope was also relabeled to "All prompts" and gained a hint clarifying it covers your entire prompt library, not just the prompts shown in the list below.
- **Every color, icon, and animation the app uses is now customizable, and the UI Legend is the editor for it** — terminal status dots, tab types, sidebar symbols, PR badges, git repo status, and diff stats were previously hand-copied into the legend as a second source of truth, which had already drifted from the real components (the busy dot's documented color was wrong, so was the open-PR badge's, a whole "Panels" section described accent colors no panel actually applied). A new registry (`src/indicators/registry.ts`) is now the single source of truth: the legend renders from it, the real components read the same `--ind-*` CSS variables it defines defaults for, and a parity test fails the build if the two ever diverge again. Settings → Appearance → UI Legend (also available read-only from Help → UI Legend) — clicking a row's own preview icon opens one combined dialog with that indicator's color swatch, a shape from 18 curated monochrome icons, and an animation from a themed set, whichever it has — live, no restart, surviving theme switches — with a reset per row and a "Reset all indicators" button. Four group headings (Tab Types, PR Status Badges, Git Repo Status, Diff Stats) additionally carry a show/hide toggle for that whole group.
- **Git repo status is now shown in the sidebar and the Changes tab** — a worktree (including the main checkout, which was previously invisible to this detection) with a rebase, merge, cherry-pick, revert, or bisect in progress now shows a distinct colored badge naming which one, instead of the previous single generic "Rebasing" label that could misname a merge or cherry-pick and never showed for the main checkout at all. The Changes tab separately shows a conflicts banner listing every unmerged file when the working tree has one, regardless of which operation caused it — the backend already computed this list, but the frontend silently dropped it until now.
- **Remote/LAN access now defaults to HTTPS even without Tailscale** — Some browser APIs (e.g. the async Clipboard API) are only exposed in a secure context, so a plain `http://<lan-ip>` remote-access URL silently lost that whole class of feature. When Tailscale HTTPS isn't active (or fails to provision), TUICommander now generates and serves a self-signed TLS certificate instead of falling back to plain HTTP — covering `localhost` and every current LAN IP, cached under the config directory, and regenerated automatically on expiry or when the machine's LAN IPs change (checked at boot/restart and via a background re-check loop that also catches a mid-session network change, e.g. roaming to a new Wi-Fi network). Plain `http://` requests on the same port now 301-redirect to `https://` while the self-signed fallback is active, so existing bookmarks and QR codes keep working. Settings > Services > Self-Signed HTTPS shows status, expiry, a SHA-256 fingerprint for verifying the browser's warning dialog before clicking through, and a Regenerate action. Tailscale HTTPS still wins whenever it's available — zero-warning beats one-warning.
- **Connect via `.local` name on macOS** — The self-signed cert's SAN list and the network picker (QR dialog, Settings → Services) now also include the machine's existing Bonjour/mDNS hostname (e.g. `MyMac.local`) — the same name AirDrop and file sharing already use, with no new mDNS infrastructure of its own. Pick the new "mDNS" entry to connect over `https://MyMac.local:<port>` instead of a raw IP that might change after a network switch. Windows/Linux are unaffected — neither guarantees an mDNS responder is running for the hostname out of the box.
- **Command block gutter click now also folds** — clicking the fold chevron on a block's header row folds/unfolds it; clicking anywhere else in the block's gutter still selects its output for copying, as before. The gutter itself is also wider (6px → 14px) with a pointer cursor on hover, for an easier click target.
- **Block-scoped search shows which block it's scoped to** — `Cmd+Shift+B` used to resolve the target block silently; a thin accent bar now marks it along the terminal's left edge.
- **Block timestamp display is now a 3-way mode instead of an on/off toggle** — Settings > Terminal > Blocks > "Show block timestamps" is now off / hold-`Ctrl+Cmd` / always, instead of only gating the hold-to-reveal behavior on or off. Existing configs migrate automatically (`true` → hold-`Ctrl+Cmd`, `false` → off).
- **Shell integration setup for bash and fish** — Settings > Terminal > Shell Integration shows a copyable one-line snippet for each (zsh remains fully automatic, no setup needed); `docs/user-guide/terminals.md` documents the setup and compares it to iTerm2's own per-shell installer.
- **"Copy ignored files"/"Copy untracked files" now actually copy something, and gained a repo-specific "always copy these files/directories" list** — both settings were previously fully wired through persistence, per-repo tri-state override, and the settings UI, but had no consumer anywhere: `git worktree add` only checks out tracked content, so turning either toggle on did nothing. A new background copy engine now runs after a worktree is created (desktop and MCP/HTTP paths alike), enumerating untracked/ignored files via `git ls-files` and copying them in; a toast marks when the sync starts and finishes. Settings → Repository → Worktree also gains an "Always Copy These Files/Directories" list — repo-specific, no global default — where each entry can be a full recursive copy or a symlink back to the source repo's copy (e.g. to share one `node_modules` across worktrees instead of duplicating it), applied independently of the two toggles.
- **Command Palette gains scope chips for filtering by result type** — a row of six chips (All / Actions / Prompts / Files / Content / Terminals) below the search box narrows the list without retyping a query. All/Actions/Prompts filter command-mode actions by category (Actions hides Smart Prompts entries, Prompts shows only them); Files/Content/Terminals select the existing `!`/`?`/`~` search modes by rewriting the query's prefix instead of adding a second axis of filtering. `Tab`/`Shift+Tab` cycle through all six, wrapping at either end, and carry whatever you've already typed across the switch; typing a prefix directly still lights up the matching chip.
- **New `md-kanban` plugin: a kanban board over checkbox tasks in a plain markdown file** — an alternative to `wiz-kanban`'s stories/plans/reviews directory convention for anyone who already tracks tasks as `- [ ]` lines in a single note. Inline `[field:: value]`/`(field:: value)` metadata drives priority (shown as an emoji and used to order each column), a `dependsOn`/`id` dependency graph (`<`/`>` badges highlight upstream/downstream cards on hover, with a `!` warning when an upstream task is behind the downstream one — an upstream in Blocked always warns, regardless of the downstream's own state), and `#tags` plus the heading path a task sits under, both shown on the card. Dragging a card between columns rewrites the task's status character in place and adds/removes a `completion`/`cancelled` date field; an "Hide archived (>5d)" checkbox filters old Done/Won't-Fix cards client-side without touching the file. Add multiple boards via a native file picker, switch between them with an in-panel tab strip, and open the underlying markdown file directly from the board. Two new plugin-host capabilities ship alongside it for any plugin to use: `ui:external-link` (open an http/https/mailto link from a panel) and `ui:file-picker` (show the native "Open file" dialog).
- **Inline images render directly in the terminal** — both iTerm2's OSC 1337 `File=` protocol and the Kitty graphics protocol are now parsed and painted on their own canvas layer, above the text and below the cursor/selection. Covers neovim image plugins (`image.nvim`, `snacks.nvim`), `yazi`'s previewer, `mpv`/`timg` video playback, and any script using iTerm2-style `imgcat` — including TUICommander's own clean-room `tuic imgcat`/`imgls`/`divider` CLI reimplementations of those three scripts. Supports Kitty transmission, transmit-and-display, and placement (including behind-text `z<0` placements and Unicode virtual placeholders, `U=1`) with full z-order compositing against text. See the Terminal Features guide's "Inline Images" section.
- **Remote Servers: Local transport, a shared Test Connection command, and SSH remote-daemon provisioning** — SSH Tunnels and Remote Machines are now one "Remote Servers" settings tab. A new Local transport connects to another TUICommander instance on the same machine (no SSH/network needed); every connection type (SSH, Direct, Local) gains a shared "Test Connection" command; Direct connections now work against a self-signed/untrusted certificate via a local TOFU-pinned proxy instead of failing outright; and an SSH connection whose far end has no `tuic-remote` running can now be offered an install/start (with OS/arch auto-detection) and, if unconfigured, a password set via the OS keyring — every remote-state-changing step gated behind an explicit confirmation. See the Remote Access guide and `docs/features/ssh-tunnels.md`.

### Fixed

- **Home/End/arrows could drop a zsh vi-mode (`bindkey -v`) shell into vi-cmd-mode instead of moving the cursor, and an app-requested block cursor was silently overridden by your own cursor-style setting** — the terminal always sent plain CSI sequences for arrows/Home/End regardless of whether the app had requested application cursor keys (DECCKM, which zsh's `zle` enables on every prompt); an unbound CSI sequence's leading ESC gets consumed as `vi-cmd-mode` under `bindkey -v`. DECCKM is now honored (SS3 sent instead of CSI while active), and an explicit DECSCUSR block-cursor request now wins over your setting, matching iTerm2/Alacritty/kitty/WezTerm.
- **The cursor didn't widen over a wide glyph (CJK, emoji), and DECSCUSR's blink/steady distinction was ignored** — a block/underline cursor now covers the full width of a wide character that bleeds into the next cell, and an app that requests a steady cursor (`\x1b[2 q`) now gets one instead of always blinking.
- **Click-and-drag text selection didn't work in Claude Code, vim, lazygit, and other apps that track their own mouse-drag selection** — any-motion mouse tracking (`?1003h`) always reported SGR button code 3 ("no button") on every mouse-move event, regardless of whether a button was actually held, so an app watching for a real drag saw indistinguishable hover events and never extended its selection. The held button is now reported correctly. TUICommander's own local Shift+drag selection fallback was also hardened: a gesture claimed as local at mousedown now stays local for its whole lifetime, and double/triple-click-drag now extend by word/line instead of degrading to plain character selection.
- **File Browser and Git Panel were hidden on every tab of a repo with "auto-consolidate worktrees" turned on** — both panels are suppressed while the manual, potentially cross-repo Global Workspace is active (since neither panel has one well-defined repo to show there), but a per-repo auto-consolidated workspace always does have exactly one repo and was wrongly triggering the same suppression.
- **The Finder "New TUICommander Tab Here" service could fail with "The Service cannot be run because it is not configured correctly"** — the installed Automator bundle carried no code signature at all, which Gatekeeper rejects when Finder's Services menu dispatches it. It's now ad-hoc signed as part of installation; if you hit this, remove and re-add Finder Integration in Settings > General to install a freshly signed copy.
- **Claude Code showed as Idle while it was still waiting on a backgrounded tool call** — Claude can end its own turn while a task it started in the background (e.g. a `run_in_background` Bash command) keeps running; TUICommander's existing background-work detection is a generic OS process-tree scan that deliberately shows "Idle" once the composer is ready (so a Codex-launched long-lived dev server doesn't permanently latch a row "Working"), so this specific case rendered as Idle too. Claude Code's own `Stop` hook already names any such outstanding task — `tuic-hook` now scrapes it, and TUICommander shows "Working" for this authoritative, Claude-self-reported case without touching the existing Codex carve-out.
- **Toast sounds ignored every notification setting** — a toast raised with its `sound: true` flag (in-app UI toasts like a failed stage/discard/merge, and the plugin `tuic.toast()` API) played a separate, hardcoded synth with no connection to Settings > Notifications: muting all sounds, changing the volume or output device, or choosing a preset/custom file for Info/Warning/Error did nothing for these toasts. They now route through the same customizable sounds as everything else, matched by level (`info`→Info, `warn`→Warning, `error`→Error).
- **The Command Palette's "Prompts" scope chip always rendered empty** — no built-in Smart Prompt ever declared the `command-palette` placement the palette filters on, so it only ever showed a prompt you had manually opted in yourself. Every built-in now ships with that placement enabled by default; a version-gated migration also applies it retroactively to an already-installed, otherwise-unmodified built-in on next launch (a placement you'd deliberately customized on such a prompt — e.g. unchecking "Toolbar" — is reset once as part of this one-time migration).
- **Picking a prompt from the Prompt Library always force-opened the Compose box, even when it was closed** — inject-mode prompts with no explicit Target now default to **Auto**: fill the Compose box if one is already open, otherwise write straight to the terminal input, instead of always popping Compose open. Both the Prompt Library dialog's own edit form (which previously had no Target field at all) and Settings > Smart Prompts now expose Auto/Compose box/Terminal explicitly.
- **The Prompt Library dialog's Tab key moved focus through Search and the category buttons instead of staying in Search** — Tab/Shift+Tab now cycle the category chips (All/Custom/Recent/Favorites) instead, mirroring the Command Palette's scope-chip cycling; the footer's key hints were also restyled to match the Command Palette's `<kbd>`-chip footer instead of a single line of text.
- **The Placement checkboxes in both prompt editors showed raw values like `git-changes` and `terminal-context` with no explanation of where they surface** — both now show a user-friendly label (e.g. "Git panel — Changes tab", "Terminal right-click menu") with a hover description of the exact surface; the Prompt Library dialog's own editor was also missing the `file-context` placement entirely and now includes all eight.
- **A Shell script, Headless, or API-mode prompt picked from the Prompt Library dialog was injected as plain, unresolved text instead of actually running** — the dialog now dispatches by execution mode, the same way every other placement (toolbar, Command Palette, context menus) already did. A failed execution now also surfaces a toast (previously logged only, with no visible feedback), using the same friendly error wording (e.g. "Stage some files first") the toolbar's SmartButtonStrip already showed.
- **The Prompt Library dialog could leak Escape to the terminal underneath, and stray keystrokes could get redirected into the active terminal while it was open** — it's now registered with the app's central modal stack like every other dialog.
- **Keyboard selection in the Prompt Library dialog reset to the top on every favorite/enable toggle, and never scrolled the selected row into view** — selection now persists across unrelated store writes and follows the keyboard cursor into view.
- **Clicking in a terminal that has enabled its own mouse-click-to-reposition-cursor feature (e.g. Claude Code CLI's prompt) could show a stuck hand cursor over ordinary text, instead of the text-beam** — the command-block gutter's hover affordance (added alongside the gutter widening above) set the mouse pointer to a hand directly whenever the cursor was over the leftmost strip, but two paths never reset it back: leaving the gutter for plain text relied on a 100ms-debounced link-hover check that only clears a *detected link*, not a bare gutter hover; and once the app enables xterm mouse reporting, terminal mousemove handling returns before reaching any cursor-updating code at all, freezing whatever the pointer happened to be at that moment (very plausibly "hand", from having merely drifted over the gutter earlier) for the rest of that app's session. Both paths now force the cursor back to the text-beam.
- **A hook-instrumented Claude Code session's tab could latch "awaiting input" forever after it had genuinely finished and gone idle** — Claude Code's `Notification` hook fires for 12 different reasons (a real permission/approval prompt, an MCP elicitation dialog, a background session finishing, quota auto-resume, and — the culprit here — its own ~60-second idle-timer heartbeat once you haven't typed anything since the last response), and every one of them was treated as an equally confident "the agent needs your input" signal. The idle-timer heartbeat now gets classified deterministically by Claude Code's own `notification_type` field: a genuine block still confidently badges as before, purely informational fires (auth succeeding, a background session completing, …) never badge at all, and the idle-timer heartbeat is dropped outright once your session has actually gone idle after a real completion — with a low-confidence, self-clearing fallback for the (rarer) case where it fires while still legitimately stuck mid-turn.
- **The Headless Agent select in Settings > Providers and Settings > Smart Prompts appeared to forget your choice, but it was actually saving fine** — the `<select>`'s options come from agent binary detection, which is async and starts empty; binding the select's own `value` (rather than each `<option>`'s `selected`) applied once, before detection resolved, and never re-applied once the options arrived — so the select got stuck showing "— Not configured —" on every reopen even though the correct value was on disk. Also fixed a related, genuine save bug in the Smart Prompts copy of this control: choosing a named per-agent run config (rather than an agent's default) silently reset the setting to "not configured," because the value it needed to store (`"agentType:configName"`) was rejected by a guard written for a plain agent type. A run config whose own name contains a colon (nothing prevents one) also had that name silently truncated when resolving which config to run — JavaScript's `split(sep, limit)` truncates the full split's result array rather than doing a max-`limit`-parts split, which is not what the code intended.
- **The New Worktree dialog resized while you typed** — the "Start from" base-ref picker, the status line, the path preview, and the error message were all conditionally mounted, so each one appearing or disappearing (e.g. the moment a typed name became an exact match for an existing branch) changed the dialog's height. The base-ref picker now stays mounted and disables instead of unmounting; the status line, path preview, and error message keep their existing conditional behavior but now live inside a fixed-height wrapper, so the dialog's overall height no longer changes no matter which combination of them is currently showing.
- **Tab-focusing a dialog's non-input control (e.g. the New Worktree dialog's "Start from" picker) and typing sent the keystroke into the terminal underneath instead of the control** — the global keyboard-redirect hook that forwards printable keys to the active terminal only ever excused a focused `<input>`/`<textarea>`/`<select>`; a focused `<button>` (or any other dialog control) was fair game. It now also bails out whenever a modal is registered, closing the whole class of "dialog control that isn't a plain input" rather than just this one case. The "Start from" trigger is also now itself keyboard-usable: Enter/Space/↓ open its list, and typing a character opens it and starts filtering immediately — including a second character typed fast enough to land before the search box has actually taken focus, which used to be silently dropped.
- **The matching-branches list in the New Worktree dialog appeared under the unrelated "Start from" field** — it now renders directly under the worktree-name field it actually filters, with the "Start from" row moved below it. Its dropdown now picks whichever direction — up or down — has more room each time it opens (measured from the trigger's actual on-screen position), instead of always opening upward and risking the same clipping problem at the opposite edge on a short window.
- **The Edit Prompt dialog's text box was too narrow to comfortably edit a real prompt** — the dialog is now noticeably wider than the Prompt Library drawer behind it (so it reads as a layer above, not a narrower inset) and the Content field is taller, with the whole dialog scrolling internally instead of only growing downward.
- **`autofocus` wasn't reliably focusing several dynamically-shown fields** (the Edit Prompt dialog's Name field, the Prompt Library drawer's search box, the Knowledge history overlay's search box, and a repository group's rename field) — native autofocus-on-insertion support varies by WebView, which is why other dialogs in the app (New Worktree, Rename Branch, the branch switcher) already use an explicit focus call instead of the bare HTML attribute. All four now do the same and actually focus when they appear.
- **"Smart Prompts Library" is now just "Prompts Library"** — the drawer's heading and its toolbar-button tooltip matched the feature's actual scope (built-in and custom templates of any kind, not only the automation-focused "Smart Prompts") poorly; both now read "Prompts Library".
- **Opening the Edit Prompt dialog while the Compose panel was open looked rendered but was dead to clicks** — Compose's `focusout` handler unconditionally reclaimed focus one frame after it fired, regardless of where focus actually went, so a click into any field of a dialog opened over Compose lost focus back to Compose on the very next frame. It now only reclaims when focus fell through to `<body>` (which would otherwise silently drop keystrokes) or while a modal is open, leaving focus alone anywhere else.
- **Triggering a Smart Prompt while the Compose panel was already open silently did nothing — until the panel was later closed and reopened, at which point it overwrote whatever had been typed in the meantime** — Compose only re-seeds its content on the closed→open transition, so setting new text while it was already open was a no-op until the next open. A prompt triggered this way now appends to the existing draft (with a blank-line separator) instead of being silently dropped.
- **Typing in a Smart Selection rule's "Menu label" (or Name, or Pattern) field in Settings scrolled the whole settings page back to the top after one keystroke** — every field edit replaced the touched rule's object in the store, and a reference-keyed list re-mounted that row (destroying the focused input) in response; when the underlying rule set had never been customized, the *first* edit also replaced every other rule's reference at once, remounting the entire list. The rule list now keys by position instead of by object reference, so an edited value updates in place without tearing down and rebuilding the row.
- **Settings → "Selection" is now "Smart Selection"** — the tab's label didn't match how the feature is described everywhere else (Smart Selection rules, the terminal's own right-click menu). Only the display label changed; the tab's internal id (and the `tuic://settings?tab=selection` deep link) are unchanged.
- **`tuic-remote` (the headless daemon) ignored a manually configured TLS certificate** — `run_remote` computed `tls_config` from `services.tls` in `config.toml` but never actually threaded it into the serve call, which was unconditionally plain HTTP regardless of configuration. It now branches into dual-protocol HTTP+HTTPS the same way the desktop server does when a cert is configured.
- **A pane whose tab content died without ever firing a session-closed event (e.g. a tmux-shim-materialized Agent Teams pane) could wedge a split open forever** — the pane never reached zero live tabs, so it couldn't close itself, any new tab opened while it was active landed inside it, and "Reset Panel Sizes" couldn't clear it either, since Global Workspace was silently reapplying its own stale cached layout on top. A dead ("ghost") tab is now detected and swept the moment its underlying session/document is actually removed, and Reset Panel Sizes now also rebuilds Global Workspace's cached layout instead of only resetting the pane-size store.
- **A Claude Code Agent Teams swarm spawned via the `tuic`-as-`tmux` shim could land multiple teammates in the wrong directory, dead on arrival** — if the calling shell had transiently `cd`'d elsewhere at the exact moment `tmux new-session`/`split-window` fired, every teammate inherited that one bad directory reading via topology inheritance, since cwd resolution fell back to the live OS cwd at that instant rather than the PTY's own stable, spawn-time worktree path. `resolve_cwd()` now prefers the calling process's own `TUIC_WORKTREE_PATH`/`TUIC_MAIN_REPO_PATH` (fixed at PTY spawn, never touched again), and the server's pane-materialization now falls back to a pane's already-recorded topology cwd when a request (e.g. `respawn-pane`, which structurally never sends one) doesn't supply its own.
- **The remove-worktree confirm dialog always claimed the local branch would be deleted, even with "Delete local branch when removing worktree" turned off** — the dialog's message now reflects the setting's actual effective value instead of an unconditional claim.
- **Removing a worktree with an initialized submodule failed outright in the new safe removal mode** — `git worktree remove` refuses any worktree containing a submodule, even a clean one ("working trees containing submodules cannot be moved or removed"), and only the safe (no-`--force`) path added above could hit that refusal. It now retries with `--force` specifically for this case, after replicating the same dirty-worktree check itself first.
- **Removing a worktree can no longer destroy live agent work** — `git worktree remove` was always called with a single `--force`, which silently overrides git's own dirty-worktree refusal on every removal path, and branch deletion escalated to `git branch -D` on the same flag that overrode the worktree's safety checks — the combination that let a worktree with five unpushed commits and an open PTY session get deleted twice out from under an active agent. Removal now defaults to a genuinely safe mode (no override at all): a dirty worktree or a `git worktree lock`ed one is refused with a distinct error, and the UI offers a specific, Enter-safe ("Delete anyway", defaulting to Cancel) confirmation for each before retrying. A live PTY/agent session attached to the worktree is refused outright with its own "in use" confirmation shown *before* any terminal is closed — this is checked independently of git's own state, since a clean, unlocked worktree can still have a terminal open in it, and it's what a batch delete now gates per item instead of queuing one identical "are you sure" per selection. Branch deletion always uses the safe `git branch -d`, never `-D`, no matter how the worktree itself was removed. Worktrees are also now git-locked for the lifetime of an attached session (unlocked on detach, swept for stale locks left by an unclean shutdown), and the app-side log entries that used to only reach a 1000-entry in-memory ring buffer now also persist to the daily log file, so a click that starts a removal survives long enough to investigate.
- **Terminal ID generation no longer throws on a non-secure context** — `crypto.randomUUID()` is unavailable outside HTTPS/localhost; every call site now goes through a shared `randomId()` helper with a `crypto.getRandomValues()` fallback and, as a last resort, a non-cryptographic one, so remote/browser-mode access over plain HTTP no longer crashes ID generation.
- **Claude Usage Dashboard shows usage for enterprise/spend-based plans** — these accounts don't populate the standard 5-hour/7-day rate-limit buckets, so both the status-bar ticker and the dashboard's Rate Limits section previously showed "no data." Both now fall back to the `extra_usage` figure enterprise plans do send, without masking a genuine bottleneck if a named bucket is populated but low.
- **Browser-mode clipboard failures are now handled gracefully** — copy/paste falls back to `document.execCommand('copy')` (with focus restore and WebKit `.select()`/`setSelectionRange` handling) instead of throwing when the modern Clipboard API is denied or unavailable in a non-secure/insecure-context browser session. The OSC 52 "clipboard updated" toast now only fires after an actually successful write.
- **Erasing or scrolling while a reverse-video pen was active dropped the highlight instead of extending it** — `EL`/`ED`/`ECH`/`DCH`/`ICH` and scroll/insert-line fills only ever propagated the pen's background color into the blank cells they wrote (background-color-erase, but only the background half of it), so `tput smso; tput el` painted a plain gap instead of a highlighted bar to the edge, unlike xterm/VTE. Blank cells now also carry the pen's reverse flag and swapped foreground; underline and strikeout deliberately do not ride along, since terminals disagree on whether background-color-erase extends those.
- **Orphan worktree cleanup was an unrecoverable hard delete, with no opt-out** — Orphan detection (detached HEAD, no branch) is a heuristic that can misclassify a worktree that was never actually abandoned, but "Ask" and "Auto-remove" both ran `git worktree remove --force` regardless, in a Tauri command and a hand-duplicated MCP HTTP route. Both cleanup modes now archive (move aside into `__archived/`, recoverable) instead; a new explicit "Auto-remove (delete, no archive)" mode covers anyone who genuinely wants the old behavior. The MCP HTTP route now shares its logic with the Tauri command instead of duplicating it. A worktree with a rebase, merge, cherry-pick, revert, or bisect in progress also now shows a **Rebasing** badge in the sidebar and is refused by the automatic merge-cleanup flows (Merge & Archive, auto-archive-merged) until the operation is resolved.
...
- **TUIC's own scrollback could accelerate past 5x on a long momentum fling** — The wheel/touch gesture-acceleration ramp had no upper bound and never reset on direction reversal, so scrolling back after an outbound fling stayed accelerated by distance traveled in the opposite direction. The ramp is now capped at 2x and restarts whenever the gesture changes direction.
- **Forwarded SGR mouse reports could corrupt a submitted prompt with literal escape-code fragments** — TUIC's internal keystroke-reconstruction buffer (used for busy-detection and prompt capture) didn't recognize `<` as a CSI private-parameter-prefix byte, so an SGR mouse report (`ESC [ < Cb ; Cx ; Cy M`, sent whenever mouse tracking is on — e.g. every wheel notch or mouse-move motion report) got misparsed: the `<` terminated the sequence early, and the remaining digits/semicolons/final-byte were spliced into the reconstructed line as literal characters. Affected any session with mouse tracking enabled, most visibly ones that shelled out to a subprocess (`less`, `top`) that doesn't itself consume mouse escapes. The parser now recognizes the full CSI parameter-byte range (`:`, `<`, `=`, `>`, `?`), so these sequences are silently discarded end-to-end like other unrecognized CSI commands.
- **Command blocks are now detected from the agent's own busy/idle signal, not scraped tool-call text** — For Claude Code the old `⏺ ToolName(args)` heuristic missed Bash and Read entirely (they don't render that prefix) and matched CC's own follow-up prose, producing one wrong-shaped block per tool call at best. Blocks are now synthesized from the idle→busy / busy→idle edges of the hook signal TUICommander already installs — one block per prompt+output turn, independent of CC's terminal rendering, with the `⏺` heuristic kept only as a fallback for agents without hook instrumentation.
- **The green user-prompt scrollbar tick fired once per tool call instead of once per prompt** — A broad, redundant `PreToolUse` hook re-affirmed "busy" before every tool call, and a second consumer of that signal emitted the tick unconditionally on each occurrence. The redundant hook entry is removed and the tick now only fires on a real idle→busy transition.
...
- **Every per-repository setting was silently dropped on save** — `repoSettings.ts` posted its store objects to `save_repo_settings` verbatim (camelCase keys: `displayName`, `copyIgnoredFiles`, …), but Rust's `RepoSettingsEntry` declares snake_case fields with `#[serde(default)]` on every one, so any key it didn't recognize — which was all of them except `path` — vanished without error. A repo's display name, color overrides, and every worktree/PR-visibility override never actually survived a restart. Save and hydrate now convert explicitly at the Tauri boundary, matching every sibling settings store. Also added the missing `pr_hide_drafts`/`pr_hide_conflicting`/`pr_hide_ci_failing` fields to the global `AppConfig` (they had no backing field at all, so the "Hide Draft PRs" toggle and friends never persisted either) and to `RepoSettingsEntry`, plus a `terminal_meta_hotkeys` per-repo field. `RepoSettingsEntry` now also carries a flattened `extra` catch-all field, so any *future* key mismatch is captured instead of vanishing the same way — `load_repo_settings()` logs a warning naming the repo and the exact unrecognized keys instead of staying silent.
- **Some terminal apps (e.g. the `leaf` markdown pager) printed a raw `ESC[…R` escape sequence instead of rendering** — TUICommander answers a cursor-position query (`CSI 6n`, DA1/DA2, DECRQM) by writing the reply back into the child's stdin, but that write only happened after an expensive per-chunk screen redraw, racing tools that set a short internal deadline while waiting for it (most visibly on a session's very first, largest paint). The reply is now flushed immediately, before that redraw work — and without holding the terminal-state lock across the write itself, so a stalled write (e.g. a backgrounded/standby session) can no longer stall that tab's live rendering while it waits. A related case — a query buried inside a synchronized-update block that never received its closing sequence — could leave a reply queued indefinitely; it's now flushed by the same 150ms stall-recovery path that already unwedges the screen itself.
- **Gutter click on a command block could select the PREVIOUS block's output** — clicking exactly on the boundary row between two blocks resolved to the older block, because `findBlockAtViewport`'s end bound was inclusive while a block's `endLine` is numerically equal to the next block's `promptLine` (both OSC 133 markers land on the same buffer line). The bound is now exclusive, matching this function's own doc comment and the equivalent bound `filterMatchesToBlock` already used for block-scoped search.
- **Folding a command block only dimmed its output instead of hiding it** — the folded region was painted at 85% opacity on the overlay canvas, leaving the text underneath faintly readable. It's now fully opaque, so a folded block's output is genuinely replaced by its "N lines folded" summary.
- **The command-block gutter never showed a success mark** — a failed shell command (non-zero exit code) drew a red bar next to its prompt line, but a successful one (`exitCode === 0`) drew nothing at all. It now draws green.
- **Unchecking "Search in Block" moved the active search match** — toggling the block-scope filter always recomputed the active match from viewport proximity, even when the previously-active match was still present in the new (unscoped) results. It's now preserved by value across the toggle, falling back to viewport proximity only when the match genuinely isn't in the new list.

### Added

- **The window and your terminal tabs reopen where you left them** — The main window's size and position are now restored on launch (Settings > General > "Restore window size and position on launch", on by default), owned entirely outside `tauri-plugin-window-state` (which still handles every other window) to work around a `set_size`/`outer_size` inner-vs-outer drift under `titleBarStyle: Overlay` that would otherwise compound on every restart; the restore self-corrects with one measure-and-correct step instead. Session restore also now brings back plain shell tabs, not just agent tabs (Settings > Terminal > "Restore open terminals on launch", on by default; off restores only agent tabs as before) — each reopens as a fresh live shell in its saved working directory. A new opt-in "Save terminal scrollback" setting (off by default, since it writes plaintext to the config directory) additionally persists each terminal's recent output and replays it above the fresh prompt, separated by a dim "restored from previous session" marker, capped by a configurable line count and clearable from Settings.
- **Smart Prompts import & export** — Settings > Smart Prompts now has an Export scope picker (Everything / Modified only / Custom only) and an Import flow that downloads or reads a portable `.json` file. Import shows a per-prompt review dialog marking each incoming prompt NEW or CONFLICT before anything is applied; prompts that run shell commands or call an external API import disabled pending review, and a forged `builtIn` flag on an unrecognized id is stripped so an import file can't fake the built-in badge.
...
- **Configurable link click activation** — Settings > Terminal > "Open links on" now controls how detected terminal links (URLs, file paths, `file://`, OSC 8 hyperlinks) activate: `Click` (previous default behavior), `⌘Click`/`Ctrl+Click` (iTerm2-style — underline/pointer cursor hidden until the modifier is held), or `Never` (right-click's Open/Copy-link menu only). Default remains `Click`, so existing behavior is unchanged unless the setting is changed.
- **Smart Selection** — Double-click word selection is now configurable and rule-driven, mirroring iTerm2's Smart Selection. Word boundaries can be a customizable character list (the previous hardcoded punctuation set is now the default) or a `|`-joined regex alternate list (e.g. add `https://` to include a URL's scheme in a double-click). A precision-scored smart-selection rule list — the built-in iTerm2 set plus dev-terminal extras (git SHA, `file:line:col`, semver, IPv4/IPv6, UUID, issue key, `#NNN`) — tries to select whole meaningful spans (URLs, paths, commit SHAs, …) before falling back to word selection; quad-click always tries it. Rules can carry actions (Copy, Open URL, Open File, Send Text, Run Command, Run Command in New Terminal, Ask AI) that surface in the right-click menu, with one default action per rule runnable via Option/Alt+double-click. New Settings > Selection tab.
- **On / Use global default / Off control for inheritable settings** — Every per-repo (and per-agent) boolean that can inherit a global default — copy ignored/untracked files, prompt on create, delete branch on remove, auto-archive merged, the three PR-visibility filters, Cmd+1-9 terminal hotkeys, and the per-agent "show intent as tab title"/"show suggested follow-ups" overrides — now uses a shared `TriStateToggle` instead of a plain checkbox. Previously "use the global default" was only the state you started in: the checkbox could only ever write a concrete on/off, so once touched the only way back to inheriting was the panel-wide "Reset to Defaults" button, which discarded every other override for that repo too. "Use global" is now a directly selectable state again. The control is a single pill switch matching the look of every other on/off toggle in Settings — clicking cycles Use global → On → Off → Use global, with the inherited position rendered dashed and dimmed partway along the track.
- **Create Worktree dialog's "Start from" base-ref picker is now searchable with full keyboard navigation** — the dropdown gained a search box with grouped Local/Remote sections that arrows cross transparently, and the existing-branch list gained the same arrow-key navigation plus match highlighting. The last base ref used successfully per repo is now remembered for the session and preselected next time that repo's dialog opens.
- **Activity Dashboard sorts idle terminals by recency and gained full keyboard navigation** — idle rows now sort by how recently they were active (the working group never reshuffles); arrow keys move a selection cursor, `Return` activates it, and digit keys `1`-`9` jump-and-activate a row directly — the same in the detached panel window.

### Changed

- **Terminal font, cursor, behavior, and shell settings moved into the new Terminal tab** — Terminal Font/Size/Weight, Cursor Style, and the live preview (previously in Appearance), Copy on Select / OSC 52 clipboard writes / the PTY prompt bar toggle (previously in General), and the Shell override field (previously in General, now the first section of the tab) all live alongside the block settings in Settings > Terminal.
- **Removed the separate "Enable smart selection" toggle** — it duplicated what "Double-click performs" already decided (Word vs. Smart), and having both meant one control's state didn't fully describe the feature's behavior. There is no longer a master on/off switch: "Double-click performs" governs only double-click, while quad-click and the right-click smart-selection menu continue to always try the rule list, exactly as before.
- **Widened the Smart Selection rule Name field** — it previously fell back to the browser's default input width instead of filling the row like the Pattern field beside it.
