...
- **Create Worktree dialog's "Start from" base-ref picker is now searchable with full keyboard navigation** — the dropdown gained a search box with grouped Local/Remote sections that arrows cross transparently, and the existing-branch list gained the same arrow-key navigation plus match highlighting. The last base ref used successfully per repo is now remembered for the session and preselected next time that repo's dialog opens.
- **Activity Dashboard sorts idle terminals by recency and gained full keyboard navigation** — idle rows now sort by how recently they were active (the working group never reshuffles); arrow keys move a selection cursor, `Return` activates it, and digit keys `1`-`9` jump-and-activate a row directly — the same in the detached panel window.
- **On / Use global default / Off control for inheritable settings** — Every per-repo (and per-agent) boolean that can inherit a global default — copy ignored/untracked files, prompt on create, delete branch on remove, auto-archive merged, the three PR-visibility filters, Cmd+1-9 terminal hotkeys, and the per-agent "show intent as tab title"/"show suggested follow-ups" overrides — now uses a shared three-position `TriStateToggle` instead of a plain checkbox. Previously "use the global default" was only the state you started in: the checkbox could only ever write a concrete on/off, so once touched the only way back to inheriting was the panel-wide "Reset to Defaults" button, which discarded every other override for that repo too. "Use global" is now a directly selectable position, same as On and Off. Arrow-key navigation moves DOM focus along with the selection, so the focus outline and the highlighted segment never desync.
- **Create Worktree dialog's "Start from" base-ref picker is now searchable with full keyboard navigation** — the dropdown gained a search box with grouped Local/Remote sections that arrows cross transparently, and the existing-branch list gained the same arrow-key navigation plus match highlighting. The last base ref used successfully per repo is now remembered for the session and preselected next time that repo's dialog opens.
- **Activity Dashboard sorts idle terminals by recency and gained full keyboard navigation** — idle rows now sort by how recently they were active (the working group never reshuffles); arrow keys move a selection cursor, `Return` activates it, and digit keys `1`-`9` jump-and-activate a row directly — the same in the detached panel window.
