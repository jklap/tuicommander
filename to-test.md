...
- [ ] [HUMAN] Confirm Settings → Agents → expand Claude/Codex/Goose shows the new "If your shell already defines its own ⟨agent⟩ function" control, that changing it there directly (without ever seeing the dialog) also takes effect on the next new tab, and that picking "Ask when detected" there re-arms the prompt.
