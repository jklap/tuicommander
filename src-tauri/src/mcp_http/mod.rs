...
fn shared_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/sessions/{id}/terminal/image-placements",
        ...
        .route(
            "/worktrees/warm-status",
            get(worktree_routes::get_worktree_warm_status_http),
        )
        .route(
            "/worktrees/finalize",
        ...
        if let Some(listener) = listener_result {
            ...
            if let Some(tls) = tls_config.clone() {
                ...
                Some(tokio::spawn(async move {
                    ...
                    if let Err(e) = server.set_upgrade(upgrade_http).serve(svc).await {
