...
pub(crate) enum OrphanCleanup {
    /// Auto-archive (move aside) worktree + prune. The safe default for unattended
    /// cleanup: orphan detection is a heuristic (detached HEAD + no branch) that can
    /// misfire, so this stays recoverable.
    On,
    /// Auto-delete (hard remove, no archive) worktree + prune. Opt-in only — a
    /// deliberate choice to skip the archive step entirely.
    Delete,
