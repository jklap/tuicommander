...
pub(crate) fn capture_session(
    ...
    let Some(vt_lock) = state.grid.vt_log_buffers.get(session_id) else {
        return;
    };
    let vt = vt_lock.lock();
    let fingerprint = capture_fingerprint(&vt);
    if let Some(mark) = state.session_maps.scrollback_capture_marks.get(session_id)
        && *mark == fingerprint
    {
...
mod tests {
    ...
    fn capture_session_detects_a_full_screen_redraw_that_never_scrolls() {
        ...
        {
            let entry = state.grid.vt_log_buffers.get("sess-1").unwrap();
            entry
                .lock()
