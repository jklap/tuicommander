...
// panicked on `.lock()` for the rest of the terminal's life, and
...
impl EventListener for TermEventCollector {
    ...
    fn image_by_id(&self, image_id: u32) -> Option<Arc<alacritty_terminal::term::cell::ImageData>> {
        self.image_store.lock().get(image_id)
    }

    fn forget_image(&self, image_id: u32) {
        self.image_store.lock().forget(image_id);
    }

    fn forget_all_images(&self) {
        self.image_store.lock().forget_all();
    }
    ...
    fn store_pending_image(
        ...
        Some(self.image_store.lock().store_pending(
            client_id,
            mime,
            intrinsic_width,
            intrinsic_height,
    ...
    fn queue_kitty_decode_job(&self, job: alacritty_terminal::term::kitty::PendingKittyDecodeJob) {
        self.pending_kitty_jobs.lock().push(job);
    }
    ...
    fn send_event(&self, event: Event) {
        match event {
            ...
            Event::TextAreaSizeRequest(cb) => {
                let window_size = *self.window_size_px.lock();
...
impl TerminalGrid {
    ...
    pub fn image_bytes(&self, image_id: u32) -> Option<Arc<[u8]>> {
        self.image_store.lock().bytes(image_id)
    }
    ...
    pub fn image_meta(&self, image_id: u32) -> Option<(String, u32, u32)> {
        let image = self.image_store.lock().get(image_id)?;
        Some((
            image.mime.clone(),
    ...
    pub fn set_cell_pixel_size(&mut self, cell_width: u16, cell_height: u16) {
        let mut ws = self.window_size_px.lock();
    ...
    pub fn cell_pixel_size(&self) -> (u16, u16) {
        let ws = self.window_size_px.lock();
    ...
    pub fn serialize_dirty_rows(&mut self) -> Vec<u8> {
        ...
        // so produces no damage of its own. `Term::damage()` also always damages
        ...
        // (The bell is deliberately NOT part of this comparison —
        // `mode_header_flags` excludes bit 0; a pending bell instead forces the
        // frame directly, see `force_send` below.)
        ...
        // The bell is edge-triggered (`drain_bell()` self-clears), so it forces
        // a frame of its own rising edge rather than riding the sustained-mode
        // comparison above — comparing it byte-for-byte against the cache would
        // force a *second* frame on the very next call just to report "bell is
        // off again". A mode-only change likewise forces a (possibly zero-row)
        // frame through so the frontend's cached mode state never lags the real
        // TermMode.
        let bell = self.drain_bell();
        let force_send = flags_changed || bell;
        let frame = if dirty_lines.is_empty() && !force_send {
            Vec::new()
        } else {
