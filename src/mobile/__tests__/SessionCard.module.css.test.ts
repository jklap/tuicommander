...
const css = readFileSync(resolve(__dirname, "../components/SessionCard.module.css"), "utf-8");
...
describe("SessionCard indeterminate progress sweep", () => {
	// Same fix, and same regression risk, as TabBar.module.css's indeterminate sweep —
	// see that file's test for the full rationale. Kept in sync per this file's own
	// "keep all three in sync" comment.
	const rule = ruleBody('.progressFill[data-kind="indeterminate"]');
