...
describe("compareBranches", () => {
	...
	describe("merged/closed PRs to bottom", () => {
		it("sorts merged PR branches after active branches", () => {
			const result = sortBranches([makeBranch("feature/merged", false), makeBranch("feature/active", false)], {
		...
		it("sorts closed PR branches after active branches", () => {
			const result = sortBranches([makeBranch("feature/closed", false), makeBranch("feature/active", false)], {
		...
		it("sorts merged and closed alphabetically among themselves", () => {
			const result = sortBranches([makeBranch("feature/z-merged", false), makeBranch("feature/a-closed", false)], {
	...
	describe("full sorting: main first, active middle, merged/closed bottom", () => {
		it("sorts all three tiers correctly", () => {
			...
			expect(names(result)).toEqual(["main", "feature/active", "feature/closed", "feature/merged"]);
		});
	});
	...
	describe("edge cases", () => {
		...
		it("handles branches with no PR state", () => {
			const result = sortBranches([makeBranch("feature/b", false), makeBranch("feature/a", false)]);
