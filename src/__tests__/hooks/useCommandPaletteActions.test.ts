...
describe("useCommandPaletteActions", () => {
	...
	it("exposes every static setting as a palette action that deep-links into Settings", () => {
		...
		const shellAction = actions?.().find((a) => a.id === "setting:terminal:Shell:Shell");
		...
		shellAction?.execute();
		expect(openSettings).toHaveBeenCalledWith("terminal", undefined, { section: "Shell", label: "Shell" });
	});
});
