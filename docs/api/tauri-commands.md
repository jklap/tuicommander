...
| Command | Args | Returns | Description |
|---------|------|---------|-------------|
| `create_worktree` | `base_repo, branch_name, create_branch?, base_ref?` | `{ status: "ok" \| "pending", name, path, workspace_id, branch, base_repo }` | Create a git worktree. `workspace_id` is how every later call addresses the new workspace (`remove_worktree`, `check_worktree_dirty`, `finalize_merged_worktree`) — read it from the response rather than assuming it equals `branch`, which holds for a linked worktree and not for a COW clone. `status: "pending"` means a stale directory is being cleaned up in the background; the row is a placeholder until `repo-changed` or `worktree-create-failed` fires. |
