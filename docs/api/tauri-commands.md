...
| `explain_session_state` | `session_id: String` | `Option<SessionStateExplain>` | Read-only troubleshooting dump: the ranked evidence held per axis (busy/idle/awaiting), whether `decide()` would flip the shell state right now, the `agent_state` ladder rung that produced the visible state, screen/silence-timer bookkeeping, the last notification classification, and the always-on decision trail (including rejected evidence attempts, with what outranked them). `None` if the session doesn't exist. Shares `pty/explain.rs::explain_session_state_impl` with the HTTP route (`GET /sessions/{id}/explain-state`) and the MCP `debug action=explain_state` tool — never re-derived per transport. Surfaced in the frontend via the Activity Dashboard row button and the terminal tab context menu's "Explain State…" item. |
...
| `set_pty_capture` | `enabled: bool, session_id: Option<String>` | `JSON` | Start/stop recording raw PTY bytes to `<config dir>/captures/<id>.tcap`; starting begins a fresh file. Surfaced as **Capture Session** in the tab context menu and the **Toggle diagnostics capture (active tab)** Command Palette action, both under `isPerfDebug()`. Browser parity: `POST /diagnostics/capture`. Dual-emits `pty-capture-changed` (see event catalog) so every open tab's recording badge stays live. |
...
| `terminal_image_ref_at` | `session_id, row, col` | `Result<Option<(u32, u32, u16, u16, i32)>, String>` (image_id, placement_id, tile_col, tile_row, z_index) | Inline-image tile at a viewport position, if any (color-tools plan). Mirrors `terminal_hyperlink_at`; populated by the OSC 1337 / Kitty graphics protocol handlers. Browser parity: `GET /sessions/:id/terminal/image-ref` |
...
| `terminal_image_meta` | `session_id, image_id` | `Result<Option<(String, u32, u32)>, String>` (mime, intrinsic_width, intrinsic_height) | Metadata for a previously transmitted inline image, by id — lets the frontend decode Kitty's containerless raw `f=24`/`f=32` pixel formats (mime `"raw-rgb"`/`"raw-rgba"`) by manually reconstructing an `ImageData`, since `createImageBitmap` can't sniff a container that isn't there. Browser parity: `GET /sessions/:id/terminal/image-meta` |
| `terminal_image_placements` | `session_id` | `Result<Vec<(u32, u32, u32, u16, u16, u16, i32)>, String>` (placement_id, image_id, abs_row, col, rows, cols, z_index) | Every current inline-image placement (color-tools plan, Phase 5) — the frontend's reconnect/new-client hydration query, since the live `image-placement` event only carries placements created after a listener attaches. Derived by scanning `CellExtra.image` refs (live screen + scrollback), not a side table. Browser parity: `GET /sessions/:id/terminal/image-placements` |
...
| `get_repo_structure` | `repo_path` | `RepoStructure` | Fast phase: worktree paths + merged branches + in-progress ops (Phase 1 of progressive loading) |
...
| Command | Args | Returns | Description |
|---------|------|---------|-------------|
| `create_worktree` | `base_repo, branch_name, create_branch?, base_ref?` | `{ status: "ok", name, path, workspace_id, branch, base_repo, instructions }` | Create a linked worktree; Git-ignored directories are then warmed with copy-on-write copies **in the background** (if `warm_ignored_directories` is on for this repo), not before this command returns. Parent tracked changes are not carried over. `instructions.warm_artifacts` is `{ present, status: "pending", poll, note }` — it never carries a finished `warmed_directories` count; poll `GET /worktrees/warm-status` (no separate Tauri command — same as the setup script's status, the desktop app itself just listens for the `worktree-warm-completed` event instead of polling) for the real outcome. `instructions.isolation` states that refs and objects are shared with the parent. |
| `remove_worktree` | `repo_path, workspace_id, delete_branch?, force?, override_busy?` | `{ branch_delete_warning?: string }` | Remove worktree. Default (`force: false`) is a Safe removal: git refuses both an uncommitted-work worktree (`worktree_dirty:` error) and a `git worktree lock`ed one (`worktree_locked:` error) — the UI is expected to catch these and re-call with `force: true` (double `--force`, overrides both). Independently of `force`, a live PTY/agent session attached to the worktree refuses with `worktree_busy:` unless `override_busy: true` — overriding a live session never escalates dirty-file or lock handling. `delete_branch` (default true) controls whether the local branch is also deleted; branch deletion **always** uses the safe `git branch -d` regardless of `force`/`override_busy` — a worktree removal never force-deletes the branch (see the 2026-08-26 incident writeup for why this used to be unsafe). If safe branch deletion fails after the worktree is removed, returns `branch_delete_warning` so the UI can report that the branch was kept. Archive script resolved from config (not IPC). |
...
| `remove_orphan_worktree` | `repo_path, worktree_path` | `String` | Archive an orphan worktree by filesystem path (validated against repo); returns the archive destination |
| `delete_orphan_worktree` | `repo_path, worktree_path` | `()` | Hard-delete an orphan worktree by filesystem path — no archive, unrecoverable. Only for the explicit `Delete` orphan-cleanup mode |
...
| `get_workspace_lifecycle` | `repo_path, workspace_id` | `{ dirty, commit_status, removal_safety, error? }` | Fresh workspace-id-addressed removal preflight. `commit_status` is `unmerged`, `merged`, or `unknown`; `removal_safety` is `safe`, `requires_force`, or `unknown`. Exact `HEAD` ancestry decides merged state. Unknown never authorizes removal. HTTP twin: `GET /worktrees/lifecycle`. |
| `list_base_ref_options` | `repo_path` | `Vec<{name, kind, is_default}>` | List valid base refs for worktree creation — default branch first, then local branches, then remote-tracking branches, each flagged `kind: "local"\|"remote"` |
| `run_setup_script` | `script, cwd` | `{exit_code: number, stdout: string, stderr: string}` | Run a shell script (any of Setup/Archive/Run Script) in `cwd`, injecting `TUIC_*` context (`script_env::ScriptContext`) and enforcing a configurable timeout. Errors (as opposed to a non-zero `exit_code`) only for a missing `cwd` or a timeout. Also reachable over HTTP: `POST /worktrees/run-script` (auth-gated, same response shape). |
