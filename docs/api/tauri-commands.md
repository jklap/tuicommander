...
| Command | Args | Returns | Description |
|---------|------|---------|-------------|
| `create_worktree` | `base_repo, branch_name, create_branch?, base_ref?, mode?, dirty?` | `{ status: "ok" \| "pending", name, path, workspace_id, branch, base_repo, kind?, degraded_reason?, instructions? }` | Create a workspace. `mode` (`auto` \| `cow` \| `worktree`, default **`auto`**) and `dirty` (`inherit` \| `clean_untracked` \| `clean`, default `inherit`) are the same two levers MCP and HTTP take, so the three transports agree on what `create_worktree` means — a caller that omits them gets a COW clone where one is available. `mode=worktree` deliberately keeps the original code path, because that is where the stale-directory recovery lives (`status: "pending"`); every other mode goes through `worktree::create_workspace` and returns `kind`, `degraded_reason` and the model-facing `instructions` payload. `workspace_id` is how every later call addresses the new workspace (`remove_worktree`, `check_worktree_dirty`, `finalize_merged_worktree`) — read it from the response rather than assuming it equals `branch`, which holds for a linked worktree and not for a COW clone. `status: "pending"` means a stale directory is being cleaned up in the background; the row is a placeholder until `repo-changed` or `worktree-create-failed` fires. |
| `remove_worktree` | `repo_path, workspace_id, delete_branch?, force?` | `{ branch_delete_warning?: string, branch: string }` | Remove one workspace, addressed by opaque `workspace_id` — never a branch, since two workspaces may share one (#726-5ac7). **Dispatches on the mechanism** (#733-21ca): a linked worktree goes through `git worktree remove`, which also clears the parent's `.git/worktrees` admin entry — deleting the directory directly would leave that entry behind, and it blocks a later checkout of the branch. A COW workspace is an independent clone, so its removal is gated on unpublished commits: those reachable from HEAD and from no remote and no mirrored parent ref exist ONLY there, and the refusal names the count and the way out. `force` (default **false** on every transport) removes anyway and reports how many commits it destroyed. `branch` echoes the branch read off the record before removal, which the caller cannot re-derive afterwards.; `delete_branch` (default true) controls whether the local branch is also deleted. If safe branch deletion fails after the worktree is removed, returns `branch_delete_warning` so the UI can report that the branch was kept. Archive script resolved from config (not IPC). |
...
| `get_worktree_paths` | `repo_path` | `HashMap<String, { branch, path, kind: "cow" \| "worktree" }>` | Every workspace of a repo, keyed by opaque `workspace_id` with the branch and `kind` as fields on the value. Never keyed by branch: two workspaces may share one. For a git worktree the id **is** the branch (the identity migration), so nothing persisted moves; only a COW clone carries a minted id. Before merging in the registered COW rows, this self-heals: a marked COW clone whose `repositories.json` row went missing is re-adopted here (best-effort, never fatal to the listing itself), and only then are the two id universes merged — a git worktree and a COW workspace sharing one id is a hard error. |
| `adopt_cow_workspace` | `repo_path, candidate_path, workspace_id?` | `{ workspaceId, branch, path }` | Explicitly register a markerless COW clone this backend lost track of (no provenance markers, so `get_worktree_paths`' self-heal and `remove_worktree` refuse to trust it on sight). The call itself is the confirmation — no separate force flag — because it never deletes anything and only ever writes the same two `tuicommander.cow.*` markers creation would have into the clone's local git config; HEAD, the index, the working tree, untracked files, refs and remotes are untouched. Validates canonical immediate-child containment, a real `.git`, parent-remote provenance, a checked-out branch, and no id/path collision before writing anything. `workspace_id` is optional: omitted, a fresh id is minted; supplied, that id is reused (refused if already taken) — for retrying a call whose registry write failed after the markers were already in place. Shares `worktree::adopt_cow_workspace_impl` with `POST /worktrees/adopt` and MCP `repo action=worktree_adopt`. |
...
| `publish_workspace` | `repo_path, workspace_id` | `PublishOutcome` | Get a workspace's commits into the parent repo and out to origin. A COW clone is an independent repository, so its commits exist ONLY there until this runs — and `git merge <branch>` in the parent silently takes a stale same-named ref instead. Two steps reported independently (`parent_updated`/`parent_error`, `origin_pushed`/`origin_error`): a network failure must not read as "the parent did not get it". The parent update is fast-forward only and compare-and-swaps against the ref it checked; a divergent or checked-out parent branch is refused with the reason, after the objects have already been staged under `refs/tuic/published/<id>`. A linked worktree returns `no_op_reason`: its refs are shared already. Shares `worktree::publish_workspace_impl` with `POST /worktrees/publish` and MCP `repo action=worktree_publish`. |
| `count_unpublished_commits` | `repo_path, workspace_id` | `usize` | How many commits exist only in this workspace — reachable from HEAD and from no remote and no mirrored parent ref. Always `0` for a linked worktree, whose objects live in the parent and outlive the directory. Refreshes and prunes the parent mirror first, so it costs a fetch per call; refresh failure is an error because stale refs can under-count. Shares `worktree::unpublished_commits_impl` with `GET /worktrees/unpublished` and MCP `repo action=worktree_unpublished`. |
| `get_workspace_lifecycle` | `repo_path, workspace_id` | `{ dirty, commit_status, unpublished_commits, removal_safety, error? }` | Fresh workspace-id-addressed removal preflight. `commit_status` is `unmerged`, `unpublished`, `published`, `merged`, or `unknown`; `removal_safety` is `safe`, `requires_force`, or `unknown`. Exact `HEAD` reachability decides merged state. Unknown never authorizes removal. HTTP twin: `GET /worktrees/lifecycle`. |
