...
| `get_worktree_paths` | `repo_path` | `HashMap<String, { branch, path }>` | Every workspace of a repo, keyed by opaque `workspace_id` with the branch as a field on the value. Never keyed by branch: two workspaces may share one. For a git worktree the id **is** the branch (the identity migration), so nothing persisted moves; only a COW clone carries a minted id. |
