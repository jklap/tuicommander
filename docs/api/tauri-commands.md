...
| Command | Args | Returns | Description |
|---------|------|---------|-------------|
| `create_worktree` | `base_repo, branch_name, create_branch?, base_ref?` | `{ status: "ok" \| "pending", name, path, workspace_id, branch, base_repo }` | Create a git worktree. Desktop-only and always a linked worktree: `mode`/`dirty` and the `instructions` payload live on the HTTP route and MCP (`repo action=worktree_create`), which is what an agent calls. The UI reaches COW creation through #735-55d7. `workspace_id` is how every later call addresses the new workspace (`remove_worktree`, `check_worktree_dirty`, `finalize_merged_worktree`) — read it from the response rather than assuming it equals `branch`, which holds for a linked worktree and not for a COW clone. `status: "pending"` means a stale directory is being cleaned up in the background; the row is a placeholder until `repo-changed` or `worktree-create-failed` fires. |
