...
- A second, hook-authoritative signal (`declared_background_work`) also keeps a Claude session `agent_state=working`: Claude Code's own `Stop`/`StopFailure` hook payload can name a backgrounded tool call (e.g. `run_in_background` Bash) still running as the turn ends, scraped by `tuic-hook` into the `bgtasks` OSC 7770 verb. Unlike the process-tree classification above, this bypasses the "show Idle once the composer is ready" carve-out that protects a Codex-left-running dev server — see `docs/backend/pty.md`
