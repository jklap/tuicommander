...
- **Prefer TUICommander messaging / spawning:** two independent per-agent-type settings (Settings > Agents > *agent* > Prefer TUICommander messaging / Prefer TUICommander agent spawning, both default on) controlling whether TUIC's own `agent` guidance is advertised to that agent type — messaging (register/send/inbox/wait) and spawning (`agent action=spawn`, plus the "prefer TUIC over your native subagent/Task tool" framing) independently, in any of the four combinations. Gated on **both** the MCP `initialize` instructions' prose *and* the `agent` tool's own MCP schema description (`agent_tool_definition` in `mcp_transport.rs`, resolved per connection from `McpSessionMeta::agent_type`) — a client reads the latter via `tools/list` independently of the former, so gating only the prose left the tool's baked-in description just as persuasive regardless of the setting (confirmed via a live Claude Code teammate-spawn test that still called `agent action=spawn` with both preferences off, before this was fixed). Turning either off only softens that half's guidance on both surfaces; it does not remove the corresponding actions from the schema, disable the underlying tool/API, or affect the other setting. Both preferences collapse to off automatically when the `agent` MCP tool itself is disabled (nothing left to prefer)
...
- Copy ignored/untracked files into new worktrees: two tri-state (On/Use global/Off) toggles, plus a repo-specific "always copy these files/directories" list (each entry copied fully or symlinked, independent of the toggles). The copy runs in the background after the worktree is created — a toast marks when it starts and finishes, naming how many files were skipped if any
...
- **Warm linked worktrees**: every workspace shares refs and objects with the parent. After `git worktree add`, ignored directories such as `node_modules`, `target`, and `.venv` are copied with clonefile/reflink when supported, several at a time (bounded, not unbounded); tracked paths, ignored files, nested repositories, and the destination ancestor are never copied
  - Runs in the background, not before worktree creation returns — a sidebar badge on the branch row shows "still initializing" with a tooltip naming the live copied/total progress while it's in flight, clearing once warming finishes (or on a restart mid-warm, via reconciliation against the poll endpoint plus a safety timeout)
  ...
  - MCP creation payload's `warm_artifacts` states the linked-worktree isolation semantics; warming (ignored-directory build-cache copy) runs in the background and its actual outcome is polled separately via `repo action=worktree_warm_status` / `GET /worktrees/warm-status`, not read synchronously off the creation response
  - Warming is on by default and can be turned off per-repo or globally (`warm_ignored_directories`, Settings → Repository → Worktree / Settings → Git & GitHub → Worktree Defaults), alongside the existing copy-ignored/copy-untracked-files toggles
...
- A second, hook-authoritative signal (`declared_background_work`) also keeps a Claude session `agent_state=working`: Claude Code's own `Stop`/`StopFailure` hook payload can name outstanding work still running as the turn ends — a backgrounded tool call (e.g. `run_in_background` Bash) OR a still-running Agent Teams teammate — scraped by `tuic-hook` into the `bgtasks` OSC 7770 verb. Unlike the process-tree classification above, this bypasses the "show Idle once the composer is ready" carve-out that protects a Codex-left-running dev server — see `docs/backend/pty.md`
