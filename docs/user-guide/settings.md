...
| **Allow OSC 52 clipboard writes** | `boolean` | `true` | Let terminal programs set the system clipboard via the OSC 52 escape sequence (used by tmux, vim, ssh yank-over-SSH, etc.). Because OSC 52 is honored from anywhere in the byte stream, a displayed file or log can also overwrite the clipboard — so a non-blocking "Clipboard updated" notice appears on every successful write (a failed write logs quietly instead). Disable to ignore OSC 52 entirely. |
| **Reflow scrollback on resize** | `boolean` | `true` | Re-wrap scrollback history when the terminal changes width, so old output stays readable after a side panel opens. Turn it off to leave history lines as they were written and truncate them instead. The visible screen is never reflowed either way. |
| **Allow terminal focus/attention requests** | `boolean` | `true` | Let terminal programs bring the window to the front (iTerm2 `StealFocus`) or bounce the dock icon (`RequestAttention`) via OSC 1337. Disable if a script or log spams either. |
...
MCP can also be configured automatically for a specific agent from that agent's own settings — see **MCP Integration** under [Agents Tab](#agents-tab) — a note above "Manual MCP configuration" points there.
...
Lists every client (Claude Code, Cursor, Zed, etc.) that currently has a `tuic-bridge` MCP entry written into its own config, with a **Remove all MCP integrations** button that clears them all in one action and stops TUICommander from reinstalling them — do this before uninstalling TUICommander, or each client will report a missing MCP server on its next launch. Only shown when at least one client has a bridge entry.
