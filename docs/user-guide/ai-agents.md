...
**zsh only:** if your shell already defines its own `claude`/`codex`/`goose` function (a common pattern for picking a model or profile), TUICommander's launch-flag injection above is skipped for that shell — your function wins, silently, with no signal that the flag never landed. The first time this is detected, TUIC asks whether to wrap your function (calling it, then appending the flag) or leave it alone. Your answer is remembered in **Settings → Agents → *(agent)* → "If your shell already defines its own \<agent\> function"** — a third "Ask when detected" option is always available there too, to revisit the choice. Wrapping only works if your function passes its arguments through (`"$@"`); if your function already sets its own version of the same flag, TUIC's silently wins (the CLI uses only the last occurrence of a repeated flag) — the prompt calls this out before you opt in. Applies to new terminals only, not a shell already running when the setting changes.
