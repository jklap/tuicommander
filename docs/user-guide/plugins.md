...
| **3** | Requires capability | Send input to terminals (`pty:write` — raw `writePty` or agent-aware `sendAgentInput`), open markdown panels (`ui:markdown`), play sounds (`ui:sound`), read/list/watch files (`fs:read`, `fs:list`, `fs:watch`), open external links (`ui:external-link`), pick a file with the OS dialog (`ui:file-picker`) |
