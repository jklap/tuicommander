...
Both systems can coexist — a Claude Code agent spawned by TUICommander technically has both available. By default, though, TUICommander steers it toward TUIC's own tool for both spawning and messaging (that's what the "Prefer TUICommander for peers/teams" guidance is for), and it does so in two places: the MCP `initialize` instructions' prose, and the `agent` tool's own MCP schema description (what a client reads via `tools/list`, independently of the instructions text). Both are gated on the same two **independent** per-agent-type settings — **Settings** > **Agents** > *agent* > **Prefer TUICommander agent spawning** and **Prefer TUICommander messaging** — so a given agent type can prefer TUIC for one and its own native tooling for the other, in either direction, or opt fully out of both while leaving the `agent` MCP tool itself enabled. (Earlier builds gated only the instructions prose, leaving the tool's own description unconditionally persuasive — confirmed via a live test where a teammate-spawn request still called `agent action=spawn` with both preferences off. Both surfaces are now gated together.) Turning a preference off only softens what an agent is steered toward; it does not remove `spawn`/`register`/`list_peers`/`send`/`inbox` from the schema, disable the underlying tool/API, or affect the env-var injection — the actions stay callable, just not recommended as the default path. Both preferences stop mattering automatically if the `agent` MCP tool itself is disabled — there's nothing left to prefer.
