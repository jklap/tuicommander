...
- **`Copy`/`CopyToClipboard`+`EndCopy`** — reuse the existing OSC 52 `pty-clipboard-store` event (unsuffixed, dual-emitted on both the desktop window and `/events` SSE — filter by `session_id` in the payload, not by a per-session suffix) and `handleOsc52ClipboardStore` path; no separate frontend handling. Excluded from `relay_client.rs`'s `is_relayable` — clipboard text never leaves the host via a relay.
