...
Each frame: 26-byte header + variable row data. The header ends with a `historyBase: u32` (lines evicted from the history top so far); `historyBase + (historySize - displayOffset + screenRow)` is the eviction-stable absolute index the smooth-scroll row cache keys by, so a cached row never aliases onto a different line after the scrollback cap rotates. `keyboard_flags` bits 0–4 remain the public keyboard-mode mask; bit 5 carries the active primary/alternate-screen identity and is removed before exposing `keyboardFlags` to input code; bit 6 carries DECCKM's application-cursor-keys mode (`app_cursor`, read by `keyToSequence` to choose SS3 vs CSI for arrows/Home/End); bit 7 carries DECSCUSR's steady-vs-blinking flag (`cursor_steady`). Any bit added here must also be mirrored in `canvasTerminalUtils.ts::decodeBinaryFrame` and in this paragraph — three separate bugs have traced back to this three-way split staying out of sync (`app_cursor`, `cursor_steady`, and the scrollbar-visibility condition below). Per cell: 4 bytes codepoint + 3 bytes fg RGB + 3 bytes bg RGB + 1 byte attrs bitmask = 11 bytes. Decoded in `decodeBinaryFrame` using struct-of-arrays (SoA) typed arrays — zero per-cell object allocation.
...
| Scrollbar visibility | OK | Shows when `historySize > 0`, or when there are block/prompt marks to display (`shouldShowScrollbar`, `canvasTerminalMarks.ts`) |
...
| OSC 133 command blocks | OK | `pty-osc133-{sessionId}` on desktop, `{"type":"osc133",…}` on the grid WS — both serialised from `Osc133Event`, so the payload is `{ marker, line, exit_code, on_alt_screen }` on either transport. Subscribed **only** here, and subscribed before the canvas waits for its fonts — that await used to sit between mount and subscription, which is the window the first prompt marker of a session falls in. Nothing replays either event, so a marker emitted before the terminal mounts at all is still lost; the fix closes the font window, not the whole race. `handleOsc133` is not idempotent for the `A` marker, so a second listener invents one empty command block per prompt. `line` is computed as `total_scrolled() + cursor row` (eviction-stable — 2026-09-15's scrollback-ring fix; never plateaus the way `history_size()` does once the scroll-limit cap starts evicting old lines) against whichever screen (primary or alt) is active at that instant — while a fullscreen TUI (e.g. Claude Code's default renderer) has the alternate screen active, that's a transient on-screen cursor row, not a valid scrollback anchor, since a fully-repainted alt screen never grows real history. `on_alt_screen` tags exactly this; every row-anchored consumer below reads blocks through `rowAnchoredBlocks()` (`terminals.ts`), which drops `onAltScreen: true` entries — `CommandOverview.tsx` deliberately does not, since it never dereferences row content. Because `line` is eviction-stable rather than a live viewport row, every row-anchored consumer must convert it via `evictionStableToGridRelative()` (`canvasTerminalUtils.ts`) before comparing it against a viewport/buffer-line coordinate — see `CommandBlock`'s doc comment (`terminals.ts`). |
| OSC 133 gutter decoration | OK | `paintGutterMarkers` on overlay canvas, filtered through `rowAnchoredBlocks()` |
...
| Cmd+Up/Down block navigation | OK | Reads `commandBlocks` + `activeBlock`, filtered through `rowAnchoredBlocks()` |
