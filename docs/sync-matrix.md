...
| `worktree-removed` | `{ repo_path: string, workspace_id: string }` — the workspace, not the branch: two workspaces may share a branch, so a branch cannot name the row to drop (#726-5ac7) | `state.rs notify_worktree_removed()` — called by EVERY removal path: `worktree.rs` (`remove_worktree`, `finalize_merged_worktree`, `merge_and_archive_worktree`, `delete_local_branch` — all addressed by `workspace_id`), `worktree_routes.rs` (`remove_worktree_http`, `finalize_merged_worktree_http`), `mcp_transport.rs` (`repo worktree_remove`) | `useWorktreeSwitchPrompt.ts` → `pruneRemovedWorktree()` closes the branch terminals and drops the sidebar row |
