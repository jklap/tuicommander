...
| `pty-capture-changed` | `{ enabled: bool, session_filter: string \| null }` | `pty_capture.rs`'s `set_enabled()` — the single mutation point the Tauri command, the HTTP route, and (transitively) a raw curl POST all funnel through — via `AppState::emit_dual`. Global (no `pty_session_id`), so bus-only + desktop `emit`, no per-session lane | `useAppInit.ts` → `ptyCaptureStore.applyStatus(payload)`, read per-tab by `TabViews.tsx`'s capture badge via `isRecording(sessionId)` |
| `session-closed` | `{ session_id: string, reason: string, agent_type: string \| null }` | `pty.rs`'s `emit_session_closed()` — the single funnel for every close path (`close_pty_core`, `kill_pty_core`, `cleanup_session`, reader-thread EOF), via `AppState::emit_dual`. `agent_type` is read from `session_states` BEFORE the emit, since the session-state accumulator's own arm removes that entry on this same event — reading after would race itself. Also on `event_bus` for `/events` SSE | `useAppInit.ts` → picks the short (`AGENT_TAB_AUTOCLOSE_MS`) vs. long (`REMOTE_TAB_AUTOCLOSE_MS`) auto-close timer based on whether `agent_type` parsed |
| `ai-suggestion` | `{ session_id: string, trigger_reason: string, proposed_goal: string }` | `state.rs` — the AI triage layer's proposed-next-goal call site, via `AppState::emit_dual` (D.4 — used to be desktop-only). Also on `event_bus` for `/events` SSE | Not yet consumed on the SSE side — desktop-only listener today; browser parity is available, no UI wired up yet |
| `watcher-status` | `{ id: string, status: string, fire_count: number, session_id: string \| null }` | `ai_agent/watcher.rs notify_status()` — the single funnel for a rule's 4 status-changing call sites (exhausted, burst-paused, fired, user-input-paused), via `AppState::emit_dual` (D.5 — used to be desktop-only AND gated to `#[cfg(feature = "desktop")]`, so the headless `tuic-remote` build never had it at all). `status` is `rule.status` round-tripped through serde (`"active"`/`"paused"`/`"stopped"`/`"exhausted"`). Also on `event_bus` for `/events` SSE | `WatcherManager.tsx` |
| `themes-changed` | `{}` — payload-free | `themes.rs start_theme_watcher()`'s 500ms-debounced directory watcher, via `AppState::emit_dual` (D.6 — used to be desktop-only). Also on `event_bus` for `/events` SSE | `src/themes.ts` re-fetches `GET /themes` |
...
| `worktree-warm-started` | `{ repoPath: string, branch: string, total: number }` | `worktree.rs run_worktree_warm()` — the setup chain's new **first** stage, awaited by `worktree.rs spawn_worktree_setup_chain()` before the file sync below (chain order is now warm → sync → setup script), on all three creation paths (same three as `worktree-sync-started`, since warming and sync share the one chain). Resolves `config::resolve_effective_warm_setting` itself (three-tier: per-repo > `.tuic.json` > global default) — **fires only when the setting is on AND there's at least one git-ignored directory to warm**; `total` is the real candidate count (known only once the background task has enumerated candidates — a `WorktreeWarmStatus::Running{total:0}` placeholder exists synchronously before this fires, corrected to the real count here). | `useAppInit.ts` → `repositoriesStore.setWorkspace(repoPath, branch, { warmState: { status: "warming", copied: 0, total } })`, which `RepoSection.tsx` renders as a sidebar badge on that branch row. |
| `worktree-warm-progress` | `{ repoPath: string, branch: string, copied: number, total: number, current: string \| null }` | Same producer, throttled ~150ms (always flushed on completion) — candidates copy concurrently (up to `cow::WARM_COPY_CONCURRENCY`, 4), so `current` names whichever directory *finished* most recently, not necessarily in original candidate order. | `useAppInit.ts` → updates the same `warmState` (`copied`/`total`/`current`), which the sidebar badge's tooltip renders live (`RepoSection.tsx`). Also exercised by `sse_routes.rs`'s parity test. |
| `worktree-warm-completed` | `{ repoPath: string, branch: string, warmed: number, warnings: string[] }` | Same producer, once every candidate's copy has finished (or failed — failures are non-fatal, collected into `warnings`, never abort the others). | `useAppInit.ts` → clears `warmState` (`{ warmState: null }`), which removes the sidebar badge. `RepoSection.tsx` also independently reconciles via a one-shot poll of `GET /worktrees/warm-status` on mount (for a row that mounts mid-warm with no live event yet, e.g. after a reload) and a 900s safety-net timeout, so a badge can never get permanently stuck on a dropped event. |
| `worktree-sync-started` | `{ repoPath: string, branch: string }` | `worktree.rs run_worktree_file_sync()`, always awaited (in that order, now *after* warming above) by `worktree.rs spawn_worktree_setup_chain()` before the setup script — called right after a worktree is actually created on **all three** creation paths: desktop `create_worktree` (both its immediate-success and STALE_DIR background-recreate arms), `worktree_routes.rs create_worktree_shared` (MCP HTTP), and `mcp_http/session.rs create_session_with_worktree` (previously a gap — this path never called the sync at all). Resolves the repo's effective copy settings itself via `config::resolve_effective_copy_settings` (three-tier: per-repo > `.tuic.json` > global default for the two toggles; `copy_paths` is repo-specific only) — **fires only when there is something to copy**; a repo with both toggles off and an empty `copy_paths` never emits any of the three `worktree-sync-*` events. | `useAppInit.ts` → toast "Syncing files into {branch}…" |
...
| `worktree-setup-script-completed` | `{ repoPath: string, branch: string, worktreePath: string, exitCode: number \| null, error: string \| null }` | `worktree.rs spawn_worktree_setup_chain()`, once the setup script (if configured) finishes — always *after* the file sync above completes, on all three creation paths. `exitCode` is `null` only for a spawn/task-panic failure (see `error` in that case); a non-zero exit still reports via `exitCode`, not `error`. **Fires only when a setup script is actually configured** — none of the three creation paths return `setup_script`/`setup_script_error` synchronously in their response anymore; this event is the only way the outcome is reported (an MCP client has no way to observe it today, a deliberate tradeoff for fixing the sync/script ordering — see `worktree.rs`'s doc comment on `spawn_worktree_setup_chain`). | `useAppInit.ts` → toast "Setup script failed (exit N)" / "Setup script failed: {error}" on failure, silent on success |
...
| `progress-recorded` | `{ repo_path, payload: { receipt: { status, revision, eventId }, event } }` | `mcp_transport.rs report_progress()` after the SQLite transaction commits; dual-emitted to the desktop window and `event_bus` → `/events` SSE. Duplicate and paused receipts emit nothing | Progress presentation listener is delivered by the dedicated panel/presentation story; authoritative history is already durable and recoverable without a listener |
| `mcp-confirm` | `{ request_id, title, message, origin_repo_path?, origin_session_id? }` | `mcp_transport.rs` — `ui action=confirm`, dual-emitted, plus a mobile push. It used to be a native OS dialog, which no remote human could answer; every client now gets the request and the first answer wins. Also raised by `mcp_http/mod.rs`'s `confirm_open_url` for an OSC 1337 `OpenURL` request (no mobile push; same `confirm_responses` map and `mcp_confirm_response` answer path) | `McpConfirmHost.tsx` (mounted by BOTH `ApplicationOverlays.tsx` and `MobileApp.tsx`) → `stores/mcpConfirm.ts` queue → `ConfirmDialog` |
...
When touching Setup/Archive/Run script execution, their `TUIC_*` environment, or their timeouts:
...
| `src-tauri/src/worktree.rs` | `run_setup_script`/`run_script_in_dir` (inject `ScriptContext` + enforce timeout via `run_shell_capture`), `spawn_worktree_setup_chain` (the sync → script background ordering), `resolve_archive_script` |
...
**Ordering:** `spawn_worktree_setup_chain` now runs three stages in order — warm (`worktree-warm-*` events, above), then file sync (`worktree-sync-*`), then the setup script — always awaiting each before starting the next, on all three worktree-creation paths (desktop, MCP HTTP `create_worktree_shared`, MCP HTTP `create_session_with_worktree`). All three run in the background — worktree creation itself has already returned by the time any of them happens — so each stage's outcome is reported via its own dual-emitted event/pollable status rather than a synchronous response field: warming via `worktree-warm-completed` / `repo action=worktree_warm_status` / `GET /worktrees/warm-status`, the setup script via `worktree-setup-script-completed` / `repo action=worktree_setup_status` / `GET /worktrees/setup-status`. Warming moved off the synchronous creation path in a separate change from the setup-script one (warming used to run synchronously *inside* `create_workspace`/`create_workspace_with`, before this chain even started — not as a stage of it); see `worktree.rs`'s doc comments on `spawn_worktree_setup_chain` and `run_worktree_warm` for the full rationale, including why the creation response's `instructions.warm_artifacts` no longer carries a synchronous `warmed_directories` count, and why warming got its own sibling status/poll endpoint (`WorktreeWarmStatus`) rather than being folded into `WorktreeSetupStatus` — both are documented on `state.rs`'s respective doc comments.
