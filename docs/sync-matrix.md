...
| `worktree-created` | `state.rs WorktreeCreatedPayload` — `{ repo_path: string, workspace_id: string, branch: string, worktree_path: string, kind: "cow" \| "worktree" }`. Keyed by the id the backend minted; `branch` is display data. `kind` is persisted on the row because the refresh prune spares a row missing from `git worktree list` only when it says `cow`. For a linked worktree the two are the same string (`worktree::workspace_id_of_worktree`), which is why a consumer must read the id rather than assume it (#727-2085). For a COW clone the row this event describes is ALREADY durably registered in `repositories.json` by the time this fires (`cow::register_cow_workspace`, called from `worktree::create_workspace_with` before the event) — the frontend's `setWorkspace` below is what makes THIS window's store agree with disk, not what makes the row exist (#756-cf6d) | `state.rs notify_worktree_created()` — the ONLY producer of this event; called by `session.rs` (`/sessions/worktree`) and `worktree_routes.rs create_worktree_shared` (HTTP route + MCP `repo worktree_create`). Dual-emits: desktop `emit` plus `event_bus` → `/events` SSE, both serializing the same struct. **Known gap:** `worktree::adopt_cow_workspace_impl` (Tauri `adopt_cow_workspace`, `POST /worktrees/adopt`, MCP `repo worktree_adopt`) registers a workspace the same durable way but does not call this — an adopted row reaches other open windows only on their next read (e.g. their next `worktree-list`/refresh), not live. Deliberate for this pass: adoption is a rare, explicit recovery action, not the common creation path; revisit if that changes | `useWorktreeSwitchPrompt.ts` → `setWorkspace(repo, workspace_id, { branchName, worktreePath, kind, parentRepoPath })` + switch toast |
| `worktree-removed` | `state.rs WorktreeRemovedPayload` — `{ repo_path: string, workspace_id: string, branch: string }`. Addressed by workspace, not branch: two workspaces may share a branch, so a branch cannot name the row to drop (#726-5ac7). `branch` is display data captured BEFORE the checkout goes away — nothing can resolve the id afterwards | `state.rs notify_worktree_removed()` — called by EVERY removal path: `worktree.rs` (`remove_worktree`, `finalize_merged_worktree`, `merge_and_archive_worktree`, `delete_local_branch` — all addressed by `workspace_id`), `worktree_routes.rs` (`remove_worktree_http`, `finalize_merged_worktree_http`), `mcp_transport.rs` (`repo worktree_remove`) | `useWorktreeSwitchPrompt.ts` → `pruneRemovedWorktree()` closes the workspace's terminals and drops the sidebar row |
