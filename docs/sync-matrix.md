...
| `worktree-sync-started` | `{ repoPath: string, branch: string }` | `worktree.rs spawn_worktree_file_sync()` — called right after a worktree is actually created (both `create_worktree`'s immediate-success path and its STALE_DIR background-recreate success path) AND from `worktree_routes.rs create_worktree_shared` (MCP HTTP). Resolves the repo's effective copy settings itself via `config::resolve_effective_copy_settings` (three-tier: per-repo > `.tuic.json` > global default for the two toggles; `copy_paths` is repo-specific only) — **fires only when there is something to copy**; a repo with both toggles off and an empty `copy_paths` never emits any of the three `worktree-sync-*` events. | `useAppInit.ts` → toast "Syncing files into {branch}…" |
| `worktree-sync-progress` | `{ repoPath: string, branch: string, copied: number, total: number }` | Same producer, throttled to ~once every 150ms (plus always on the final entry) by `worktree.rs`'s `spawn_worktree_file_sync` — a large ignored tree (e.g. `node_modules`) can be thousands of entries. | Not consumed by the frontend today (no progress-bar UI yet) — emitted for future use and exercised by `sse_routes.rs`'s parity test. |
