...
| `block` | `start`, `end`, or `end;<exit_code>` | Emits `ParsedEvent::AgentBlock` — the turn-level command block boundary for hook-instrumented sessions (see `handle_tuic_state`, which derives `start`/`end` from the `state` busy/idle edge rather than a hook emitting `block` directly). Every `AgentBlock`/`Osc133Event` carries `on_alt_screen: bool`, tagged fresh from `is_alternate_screen()` at the moment the event fires — `line` is `total_scrolled() + cursor row` (eviction-stable, 2026-09-15) against whichever screen (primary or alt) is active, and a fullscreen TUI's alt screen never accumulates real scrollback, so `line` there is not a valid row anchor (see src-tauri/AGENTS.md's "Command Blocks" section). `AgentBlock` also carries `from_transcript_dump`/`new_dump_generation: bool` (`fromTranscriptDump`/`new_dump_generation` on the wire) — set only by `synthesize_transcript_dump_block_events`'s `[`-dump reconstruction, never by this `state=`-driven path; drives the frontend's stale-dump-block prune on a repeat `[` gesture. |
...
| `userwrap` | `claude`, `codex`, or `goose` — anything else ignored | A side-table trigger, not a `ParsedEvent` — same shape as `bgtasks`. From the zsh deferred integration's `ask` branch (`shell_integration.rs`): the shell detected the user already has their own function for that agent and `AgentSettings::wrap_user_function` is still `None`. Calls `agent_wrap_prompt::request`, which dual-emits `AppEvent::AgentWrapPrompt` (deduped to one in-flight prompt per agent app-wide) so the frontend can ask whether to wrap the user's function. Not emitted by `tuic-hook`; the shell itself emits it directly, so the payload is a bare agent name, not percent-encoded. |
...
| `bgtasks` | percent-encoded, comma-joined per-task `status` strings (e.g. `running%2Ccompleted`), or empty | A side-table write, **not** `ParsedEvent::AgentMetadata` (that variant has no consumer). Sets `SilenceState::declared_background_work` (epoch-stamped, self-expiring — the same shape as `completion_declared`, not the OS-heuristic `SessionState::background_work`) when the decoded payload contains any status OTHER than the confirmed-terminal `"completed"`/`"failed"` (fail-safe — an unrecognized future status counts as still active, not idle); clears it only when every status is one of those two. Folded into the `agent_state` ladder (`state.rs::session_state_with_shell`) and wire-exposed as `declared_background_work`, deliberately kept separate from `background_work` so the frontend can render "Working" for this authoritative, Claude-self-reported case without inheriting `background_work`'s idle-preserving carve-out for a long-lived Codex dev server. From Claude's `Stop`/`StopFailure` hooks. |

`ccsession`/`cwd`/`transcript`/`tool`/`notify`/`notifytype`/`bgtasks` payloads are percent-encoded (RFC 3986 unreserved
