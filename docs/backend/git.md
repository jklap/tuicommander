...
**Submodule refusal retry.** `git worktree remove` refuses (`fatal: working trees containing submodules cannot be moved or removed`) when the worktree's tree contains any submodule gitlink — but a plain `--force` lifts it (confirmed on git 2.55.0: `git worktree remove --force` succeeds outright here, even on a dirty worktree), so `RemovalMode::Dirty`/`::Forced` never hit this refusal at all — their first attempt already passes `--force`. Only `RemovalMode::Safe` (no force) reaches it. It fires *before* git's own dirty check though, so `remove_worktree_internal` can't rely on git to report dirtiness separately in this case — it replicates that check itself (`git status --porcelain --untracked-files=all` at the worktree root; git's default submodule-summary behavior already surfaces uncommitted/untracked content *inside* the submodule as `M <dir>`) for `RemovalMode::Safe`, then retries with `--force`. `git submodule deinit --force`/`--all --force` beforehand does **not** help — it only clears the local checkout/config, not the gitlink entry itself (`git ls-files -s` still shows `160000 ...` after deinit), so a bare retry without `--force` still fails identically.
