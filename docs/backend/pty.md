...
**Global only, deliberately** — no `.tuic.json`/per-repo tier, the same structural opt-out `copy_paths` uses (see `src-tauri/AGENTS.md`'s Worktree File Sync section), because a committed repo file must never be able to inject shell environment for anyone who opens that repo. Validation is structural only (`config::valid_custom_env_key`/`valid_custom_env_value`: a legal env var name, no NUL byte in the value) — no name denylist, unlike `additional_readable_dirs`. This is reachable the same way every other `AppConfig` field already is (a local caller, or an authenticated remote one through the same `require_local_or_auth` gate `save_config`'s route uses) — not purely "local machine only" — but `AgentRunConfig::env`/`AgentSettings::env_flags` already grant an equal or broader capability (zero validation at all) through that identical gate, so this doesn't cross any new trust boundary. Confirmed by an independent security review of the commit that added this setting.

Key-name duplicate detection (frontend list editor and the hydrate-time sanitizer, `src/stores/settings.ts`) is case-insensitive — Windows environment variable names collide case-insensitively at the OS level, so `PATH` and `Path` as two separate entries would otherwise silently let one clobber the other at spawn time with no indication which.

**`PATH` is not special-cased — setting it here replaces it entirely, it does not prepend.** `inject_unix_terminal_env` (above) prepends the image-CLI shim dir to `PATH` earlier in the injection order; since `custom_pty_env` runs after that and calls a flat `cmd.env(key, value)`, a user-added `PATH` entry silently drops that prepend (and anything else already on `PATH`) rather than augmenting it. The Settings UI hint warns about this explicitly.
...
**The "already materialized" fast path must also run the gate, not just the fresh-spawn path.** `materialize()` records `pane.tuic_session_id` in topology *before* the gate runs (so a second concurrent caller for the same pane can find it and reuse the session instead of double-spawning) — a code review caught that this meant a second concurrent `materialize` call landing while the first call's gate was still waiting hit the fast-path `return Ok(existing)` immediately, with no wait at all, reproducing the exact race this feature exists to close for that caller. Fixed by calling `apply_pane_readiness_gate` on the fast path too, before returning — cheap when the shell is already idle (`wait_for_shell_idle`'s own fast path returns immediately), correct when it isn't. Regression test: `materialize_fast_path_still_waits_for_a_not_yet_idle_shell`.
...
**Extended to every spawn-then-immediately-write path, not just the tmux shim.** The gate initially covered only `tmux_routes::materialize` — the path Claude Code's agent-teams `respawn-pane` uses, and the one implicated in the reported incident. Code review flagged that three other callers hand a freshly spawned session id straight to something that can write into it immediately (the identical race, just via a different caller): the `ai_terminal_drive_agent` MCP tool's `spawn_session` → `send_input`/`send_key`/`drive_agent`, `ai_agent/scheduler.rs`'s cron-triggered autonomous conversations, and `ai_agent/watcher.rs`'s PR-review watcher sessions. All three funnel through the single shared `pty::spawn_session_for_agent`, so the gate was added there once (`wait_for_shell_idle` called right before it returns the new session id) rather than at each call site. The timeout bound is a single shared constant, `mcp_transport::SHELL_READINESS_TIMEOUT_MS`, reused by both `apply_pane_readiness_gate` and `spawn_session_for_agent` — not a second ad hoc `5_000` next to the first.
...
**State-regression capture:** Enable `POST /diagnostics/capture` before reproducing (`{"enabled":true,"session_id":"<id>"}`), stop it afterward, and copy the exact `<config dir>/captures/<id>.tcap` file into `src-tauri/src/fixtures/agent_prompts/`. `GET /diagnostics/capture` reports the directory and bytes written. The desktop equivalents are the `get_pty_capture` / `set_pty_capture` commands, surfaced as **Capture Session** in the tab context menu and as the **Toggle diagnostics capture (active tab)** Command Palette action, both whenever `isPerfDebug()` is on (dev by default). `pty_capture::set_enabled` — the single mutation point every entry point funnels through — dual-emits `AppEvent::PtyCaptureChanged { enabled, session_filter }` (`pty-capture-changed`) so any open tab's "recording" badge updates live regardless of which entry point (or a raw curl POST from outside the app) flipped it; `session_filter: None` means every session is being recorded, so the frontend evaluates "is this session captured" per tab from that pair rather than the event carrying a session id. TUICCAP2 records the initial terminal rows/columns and preserves input/output ordering, original chunk boundaries, and monotonic timestamps. The decoder still accepts geometry-less TUICCAP1 and legacy output-only `.raw` fixtures; replay code must provide independently observed dimensions for those files. Do not build fixtures from `/sessions/:id/output`: the bounded ring can overwrite the signal and its JSON string is lossy UTF-8.
...
second, so a hook-set value would be overwritten (and its clear path would spuriously
publish a completion) on the refresher's very next tick. Instead it's stored as
`SilenceState::declared_background_work`, shaped exactly like `completion_declared`
just above — single writer, epoch-stamped, self-expiring on the next submitted
turn, untouched by any polling loop — and folded into the `agent_state` ladder
(`state.rs::session_state_with_shell`) as an additional term alongside
`background_work`. Wire-exposed as its own `declared_background_work` field
specifically so the frontend can bypass `background_work`'s idle-preserving carve-out
for this authoritative, Claude-self-reported case (see `activitySnapshot.ts`'s
`effectiveActivityState`). Classified fail-safe, not by an exact `"running"` match:
`background_tasks[].status` is not a documented closed set anywhere (unlike
`notification_type`'s enumerated values) — only `completed`/`failed` are confirmed
terminal from real production captures, so any other status (including a future or
