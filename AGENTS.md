...
- `make check`'s "plugin tests" step is **currently expected to fail** — the `plugins` submodule has zero test files. This is known, tracked state (`c6cae61a`'s CI comment); don't spend time trying to "fix" a red `make check` caused by it. **As of 2026-09-25 the same submodule pin also fails `cargo check` outright** (not just this step) — see `src-tauri/AGENTS.md`'s expanded note.
