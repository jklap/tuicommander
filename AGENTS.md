...
- **Drive it with `agent-browser`**, always via the stealth wrapper (see global rules). `@ref` CDP clicks are trusted and work (open modals/panels/tabs); **JS-dispatched keydown is `isTrusted:false`** so app keyboard shortcuts (Cmd+P, etc.) are ignored — click UI, never synthesize keys. Use a persistent `--session <name>`, restart the browser periodically (snapshots/clicks degrade after many calls), and wrap each call in a `perl alarm` timeout (macOS has no `timeout`/`gtimeout`). **The self-signed-HTTPS fallback 301-redirects plain `http://` to `https://` even on `localhost`/`127.0.0.1`**, so a bare `open http://localhost:9877/` fails with `net::ERR_CERT_AUTHORITY_INVALID` — pass `--ignore-https-errors` before the `open` subcommand (confirmed working on agent-browser 0.37.1, on a *fresh* `--session`) or the `AGENT_BROWSER_IGNORE_HTTPS_ERRORS=1` env var; which one actually takes effect has flipped between sessions on different `agent-browser` versions, so if the first form you try still shows the cert error, close the session and retry with the other form before assuming the page itself is broken.
...
5. **Even a clean `make dev` run (no broken placeholders involved) legitimately rebuilds the
   tracked `src-tauri/binaries/tuic-<target>` sidecar** — confirmed 2026-09-10: after a fully
   working worktree (gate already green) ran `make dev` purely to visually verify an unrelated
   frontend change, `git status` afterward showed that binary as modified (a few hundred bytes
   different, not a version bump — `pnpm build:sidecar` reran because `make dev`'s
   `beforeDevCommand` always invokes it). This is expected, not a bug, but it means a `make dev`
   run for manual/visual verification can leave an unintended, unrelated binary diff sitting in
   `git status` afterward. Check `git status` for this file specifically before committing, and
   `git checkout -- src-tauri/binaries/tuic-<target>` to revert it if the rebuild wasn't the
...
## Settings Search Index — Every Setting Needs a Real DOM Id

The Settings panel's search box and its Command Palette entries (`src/components/SettingsPanel/settingsSearchIndex.ts`) jump to and highlight the matched control by `document.getElementById`. That id (`settingSlugId(label)`, from `SettingFields.tsx`) is stamped **automatically** on `SettingToggle`/`SettingSelect`/`SettingSlider`/`SettingInput`'s wrapper div — but several existing settings are rendered with hand-rolled `<div class={s.group}>` markup instead of one of those four components (usually because the control needs a section-heading `<label>` separate from its own toggle label, or a `<textarea>`/custom layout those components don't support), and hand-rolled markup gets no id at all.

**If you add or edit a setting that's indexed in `settingsSearchIndex.ts` and it isn't rendered via one of the four shared components, you must stamp the id yourself**: `id={settingSlugId(t("key", "Exact Label Text"))}` (or the plain-string form if the label isn't `t()`-wrapped) on that control's own wrapper element, using the *exact* text that also appears in the registry entry's `label` field — not the section's own heading label. `settingsSearchCoverage.test.ts` enforces this for every registered entry (reads each tab's own source and fails naming the exact broken label if neither a shared-component `label` prop nor an explicit `settingSlugId(...)` call resolves it) — run it after touching any tab file that has entries in the registry. A 2026-09-10 review found this gap affecting 15 settings across 5 tab files, all fixed the same way.

**A sentinel value sharing its namespace with real, externally-sourced data is a latent collision, not just a style nitpick.** `RepoWorktreeTab.tsx`'s "Branch From" dropdown uses the string `"automatic"` as a sentinel meaning "let TUIC detect the default branch" — fine while the dropdown was a hardcoded list, but once it was changed (2026-09-10) to render the repo's actual branches, a real branch literally named `automatic` (a legal git ref name) would render as a second `<option value="automatic">`, indistinguishable from the sentinel and permanently unselectable. Fixed narrowly by filtering any ref matching a reserved sentinel (`automatic`, `__inherit__`) out of the rendered list — a display-only fix, not a sentinel-scheme redesign (that would be a wire-format/migration concern). Any dropdown that mixes a fixed sentinel value with a dynamically-fetched, open-ended real-world value (branch names, file paths, user-typed strings) needs the same check: either namespace the sentinel so it can never collide (e.g. `__automatic__`), or filter colliding real values out of the rendered options.
...
**A backgrounded tool call Claude explicitly reports as still running gets its own signal, separate from `background_work`'s process-tree heuristic.** Claude Code's `Stop`/`StopFailure` hook payload can carry a `background_tasks` array naming a `run_in_background` tool call still outstanding as the turn ends — first noticed 2026-09-14 investigating a real session (`diff-tool`) that had launched a backgrounded `check-gate.sh` run and ended its turn to wait on it. `background_work` (the OS process-tree scan, `pty.rs`'s `background_work_from_snapshot`) already flagged this session correctly, but it can't be the wire for a hook-derived value: its process-tree refresher is demand-gated on `background_work` itself being `true` and runs every second (`process_snapshot_is_demanded`, `pty.rs`), so setting it from a hook activates the very scanner that will overwrite it — and clearing it that way also spuriously *publishes* a completion event to the orchestrator. Separately, the frontend's `effectiveActivityState` deliberately renders `background_work=true` + an idle shell as **"Idle,"** specifically so a Codex-left-running dev server doesn't permanently latch a row "Working" — reusing that field would have inherited a carve-out that's wrong for this case.

The fix follows `completion_declared`'s shape exactly (`SilenceState`, `pty.rs`) rather than `background_work`'s: `tuic-hook` scrapes `background_tasks`' raw per-task `status` strings (comma-joined, unclassified — see `crates/tuic-hook/AGENTS.md`'s rule against baking Claude's evolving vocabulary into that binary) into a new `bgtasks` OSC 7770 verb; `pty.rs`'s dispatch classifies "running" and writes `SilenceState::declared_background_work`, a single-writer, epoch-stamped, self-expiring flag no polling loop touches. Wire-exposed as its own `SessionState.declared_background_work` field (not merged into `background_work`) so the frontend can bypass the Codex carve-out specifically for this authoritative case. See `docs/backend/pty.md`'s `declared_background_work` section for the full mechanism and `docs/backend/alacritty-integration.md`'s `bgtasks` verb entry for the wire format.

**Follow-up bug, found 2026-09-16: `background_tasks[].type` is not limited to `"shell"`.** A live "ai-usage" Claude Code session used Claude Code's native Agent Teams feature to dispatch three teammates, then ended its own turn — its real `Stop` hook payload (captured via `.claude/hook-debug.log`) carried `background_tasks: [{type: "teammate", status: "running"}, …]`, and `tuic-hook`'s scraper (which discriminates only by `status`, never `type`) correctly turned that into `bgtasks=running`. `declared_background_work` was briefly `true`, then read back `false` moments later with no new prompt submitted. Root cause: `reset_suggest_memory()` cleared `declared_background_work` alongside `completion_declared`, but that method is ALSO called from `apply_working_evidence`'s "reopen a stale idle/completed turn on renewed screen evidence" path (`can_reopen_completed` is unconditionally true for `agent_type == "claude"`) — which fires every time the orchestrator's own screen shows a spinner again, e.g. it waking up to poll its still-running teammates. That is not a new turn; polling children tells you nothing about whether they finished. Every such poll silently erased an accurate `declared_background_work=true` until the next real `Stop` hook happened to re-assert it. Fixed by splitting the clear into its own `SilenceState::reset_declared_background_work()`, called only from genuine new-turn-submission sites (`note_submitted_input_with_hook`'s two branches, and the desktop keystroke line-submit path in `apply_desktop_input_bookkeeping`) — `apply_working_evidence`'s reopening no longer touches it. This is the second time a function shared between "a real new turn" and "renewed screen evidence reopens a stale turn" has needed splitting apart because the two are not the same event — audit any THIRD reset method added near `reset_suggest_memory`/`apply_working_evidence` for the same conflation before assuming it's safe to share.

**Known pre-existing test-environment leak, found investigating the above (not caused by it, confirmed via `git stash`):** `pty::tests::non_repo_cwd_gets_no_worktree_vars` and `worktree::tests::run_setup_script_does_not_set_unknown_vars` both fail on this machine/environment, each expecting a plain `tempfile::TempDir` (or a detached-HEAD fixture repo) to look like "not this repo" / "no branch," but observing this actual checkout's real `TUIC_MAIN_REPO_PATH` / branch (`wip`) instead. Ruled out: `TMPDIR` itself (confirmed via a throwaway probe test to be the normal macOS per-user tmp, with no `.git` anywhere in its ancestor chain), `find_repo_root`/`resolve_git_dir` in isolation (their own dedicated unit tests pass), and ambient `GIT_DIR`/`GIT_WORK_TREE` env leakage (checked `env`, both unset). Not root-caused further — reproduces standalone (single-test `cargo nextest run`, so not cross-test pollution either) but the actual contamination path from a fresh fixture repo/tempdir to this real repo's `.git` is still unknown. Before spending time re-investigating, confirm it's still present (`cargo nextest run -p tuicommander <either test name>`) since it may be specific to this machine's environment.
